...
\begin{tikzpicture}
  ...
  \def\R{0.32}
  \draw[thick vector]
    (O) --++ (148:3.3) coordinate (X) node[above=6,left=-6,scale=1.5] {$\vec{u}_x$};
  \draw[thick vector,<->]
    (O) ++ (-172:3.25) coordinate (Y) node[above=5,left=-6,scale=1.5]
    {$\vec{u}_y$} --
    (O) --++ (82:3.2) node[above=-1,scale=1.5] {$\vec{u}_z$};
