...
linéaires à deux points fixes $A$ et $B$. On le repère par sa position $x$.
    ...
    \begin{enumerate}
        \item Cette fois-ci, on a deux ressorts~: le premier tire dans le sens
            ...
            \[ \begin{array}{ll}
                \textbf{Poids} & \Pf = -mg\uy\\
                \textbf{Support} & \Rf = R\uy\\
                \textbf{Ressort 1} & \Ff_1 = -k(AM - \ell_0)\ux = -k (\ell_{\rm
                eq} + x - \ell_0)\ux\\
                \textbf{Ressort 2} & \Ff_2 = +k(BM - \ell_0)\ux = +k (\ell_{\rm
                eq} - x - \ell_0)\ux
            \end{array}\]
            et le PFD donne
            \begin{gather*}
...
\QR{En fait il existe entre le mobile et la tige un frottement de type visqueux
    ...
    \begin{enumerate}
        ...
        \item On sait qu'on a des oscillations amorties quand le discriminant
            ...
            \begin{equation*}
                \boxed{x(t) = \exr^{-ht/2} \left[ \cos(\wt) +
...
\QR{Donner l'allure du graphe correspondant à $v(t)$. 
    ...
    Le condensateur est initialement chargé. Soit $E$ sa tension initiale. On
    utilise l'équation~\ref{eq:wieni} pour trouver que $\dv{v}{t} (0) = \frac{i'
    (0)}{C}$, sachant qu'à $t = 0$ le circuit est équivalent à un circuit $RC$
    en décharge et qu'on a donc $i'(0) = E/R$. On trouve ainsi
    \fbox{$\dv{v}{t} (0) = \frac{E}{\tau}$}
    \begin{center}
        \includegraphics[width=.5\linewidth]{wien_carac}
    \end{center}
