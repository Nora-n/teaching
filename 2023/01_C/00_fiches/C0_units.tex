...
\begin{enumerate}
	...
	\item \textbf{La valeur} de la grandeur~: \bsw{
...
\begin{tcbraster}[raster equal height=rows]
	...
	\begin{tcb}(exem)'r'{Exemples}
		\begin{enumerate}
			...
			\bsw{
				\[
					\Ec = \int_{t_1}^{t_2} \Pc(t)\dd{t} \Ra \dim{\Ec} = \dim{\Pc} \times [t]
				\]
			}
			\vspace*{-10pt}
	...
	\begin{isd}
		\bsw{
			\begin{gather*}
				...
				\frac{\rm M\cancel{\rm L}T^{-2}}{\rm \cancel{L}}
				\\ \Lra
				\boxed{\dim{k} = \rm M \cdot T^{-2}}
			\end{gather*}
		}
		\tcblower
		\bsw{
	...
	\begin{isd}
		\bsw{
			\begin{gather*}
				...
				[T] = [\ell]^{\alpha}[g]^{\beta}[m]^{\gamma}
				\\\Lra
				\si{s} = \si{m}^{\alpha} \cdot \si{m}^{\beta} \cdot \si{s}^{-2 \beta}
				\cdot
				\si{kg}^{\gamma}
				\\\Lra
				\left\{
