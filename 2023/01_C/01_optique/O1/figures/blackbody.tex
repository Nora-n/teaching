...
\begin{tikzpicture}
	...
	\begin{axis}[
		...
		ticklabel style={scale=0.8},
		%xtick style={draw=none},xticklabels=none,
		xlabel={Longueur d'onde $\lambda$ [\si{nm}]},
		ylabel={Puissance $P$ [$\si{kW.sr^{-1}.m^{-2}.nm^{-1}}$]},
		xlabel style={at={(rel axis cs:0.5,0)},below=-1pt,font=\small},
		ylabel style={at={(rel axis cs:-0.1,0.5)},rotate=90},
		width=9cm, height=7cm,
		%clip=false
		...
		% Limits
		\node[above=-1pt,scale=0.8] at (200,\ymax) {\strut UV}; % 10 - 380 nm
		\draw[densely dashed] (axis cs:380,0) -- (axis cs:380,1.1*\ymax);
		\node[above=-1pt,scale=0.8] at (560,\ymax) {\strut visible}; % 380 - 740 nm
		\draw[densely dashed] (axis cs:740,0) -- (axis cs:740,1.1*\ymax);
		\node[above=-1pt,scale=0.8] at (920,\ymax) {\strut IR}; % 740 - 1050 nm
		...
		% MAXIMUM (Wien's displacement law)
		\addplot[black, dashed, thick,variable=T,domain=2200:4000,samples=40]
		({lampeak(T)},{planck(lampeak(T),T)});
		\addplot[black, dashed, thick,variable=T,domain=4000:5200,samples=100]
		({lampeak(T)},{planck(lampeak(T),T)});

		\node[black!80!black] at ({lampeak(3000)},{planck(lampeak(3000),3000)})
		{$\bm{\oplus}$};
		\node[black!80!black] at ({lampeak(4000)},{planck(lampeak(4000),4000)})
		{$\bm{\oplus}$};
		\node[black!80!black] at ({lampeak(5000)},{planck(lampeak(5000),5000)})
		{$\bm{\oplus}$};

		% LABELS
		\node[above left] at ({lampeak(3000)},{planck(lampeak(3000),3000)})
		{$\lambda_{\max}$};
		\node[above left] at ({lampeak(4000)},{planck(lampeak(4000),4000)})
		{$\lambda_{\max}$};
		\node[left] at ({lampeak(5000)},{planck(lampeak(5000),5000)})
		{$\lambda_{\max}$};
		\node[above=0pt,scale=0.75,red] at (1150,{planck(1150,3000)}) {\SI{3000}{K}};
		\node[above right=-1pt,scale=0.75,orange!80!black] at (740,{planck(740,4000)}) {\SI{4000}{K}};
		\node[above right=-1pt,scale=0.75,blue] at (800,{planck(800,5000)}) {\SI{5000}{K}};
