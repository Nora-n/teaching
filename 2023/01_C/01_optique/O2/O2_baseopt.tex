...
\begin{tcb}[label=prop:rl](prop){Propriétés d'un rayon lumineux}
	\begin{enumerate}
		\bitem{Propagation rectiligne}~: \psw{
			Dans un milieu TLHI, la lumière se propage en ligne droite.
		}
		\bitem{Indépendance des rayons}~: \psw{
			Les rayons lumineux n'interfèrent pas entre eux. Notamment, un rayon ne peut
			pas en dévier un autre.
		}
		\bitem{Retour inverse}~: \psw{
			Dans un milieu TLI, homogène ou non, si une source en A éclaire B, alors une
			source placée en B éclaire A.
		}
		\begin{center}
			\switch{
...
\begin{itemize}[label=$\diamond$, leftmargin=10pt]
	\bitem{Diffraction}~: voir \ref{ssec:optdiff}~;
	\bitem{Phénomènes ondulatoires}~: le modèle de rayon n'explique pas les
	interférences (voir plus tard dans l'année)~;
	\bitem{Polarisation}~: en tant qu'oscillations des champs électrique et
	...
	\bitem{Inhomogénéité}~: dans un milieu inhomogène, la lumière ne se propage
...
\subsection{Système, rayons, faisceaus.}
