...
\begin{minipage}[c]{.5\linewidth}
	...
	\psw{
		\begin{DispWithArrows*}[fleqn, mathindent=-5pt]
			...
			\Arrow{$\Hu = \Su/\Eu$}
			\\\Lra
			\Aboxed{\Hu &= \frac{1}{1+\jrcw} = \frac{1}{1+\jj \frac{\w}{\w_0}}}
		\end{DispWithArrows*}
		avec $\w_0 = \frac{1}{RC}$ la pulsation de coupure
	}
\end{minipage}
...
On trouve le gain à partir du gain en décibel en inversant la formule~:
\psw{
	\[
...
\begin{tcb*}(exem){Filtrage par RC sur C}
	...
	Pour garder les basses fréquences, on choisit une fréquence de coupure $f_0$ à
	\SI{10}{Hz}. Avec $R = \SI{1000}{\ohm}$ cela impose $C = \SI{1.6e-5}{F}$
	($\w_0 = \frac{1}{RC}$). On obtient la figure suivante~:
	\smallbreak
	\noindent
	\begin{minipage}{\linewidth}
		\centering
