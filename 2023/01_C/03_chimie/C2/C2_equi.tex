...
\begin{center}
	...
	\begin{tabularx}{\linewidth}{|l|c|YdYdYdY|}
		...
		$\xmathstrut{\rhgt}$
		Initial                            & $\xi = 0$ &
		\psw{$n_0(\ce{CH_4\gaz{}})$}       & \vline    &
		\psw{$n_0(\ce{O_2\gaz{}})$}        & \vline    &
		\psw{$n_0(\ce{CO_2\gaz{}})$}       & \vline    &
		\psw{$n_0(\ce{H_2O\gaz})$}                       \\
		\hline
		$\xmathstrut{\rhgt}$
		Interm.                            & $\xi$     &
		\psw{$n_0(\ce{CH_4\gaz{}}) - \xi$} & \vline    &
		\psw{$n_0(\ce{O_2\gaz{}}) - 2\xi$} & \vline    &
		\psw{$n_0(\ce{CO_2\gaz{}}) + \xi$} & \vline    &
		\psw{$n_0(\ce{H_2O\gaz}) + 2\xi$}                \\
		\hline
	\end{tabularx}
\end{center}
...
\begin{enumerate}
	\item Si $[\ce{Ag^{+}}]_i = [\ce{Cl-}]_i = \SI{e-3}{mol.L^{-1}}$, alors
	      \psw{
		      \[
			      Q_{r,i} = \frac{c\degree^2}{[\ce{Ag^{+}}]_i \times [\ce{Cl-}]_i}
			      = \num{e6} < K\degree
		      \]
	      }
	      et la réaction se passe dans le sens \psw{direct~: on forme du précipité.}
	\item Si $[\ce{Ag^{+}}]_i = [\ce{Cl-}]_i = \SI{e-6}{mol.L^{-1}}$, alors
	      ...
	      et la réaction se passe dans le sens \psw{indirect~: on dissout le précipité.}
\end{enumerate}
...
\leftcenters{%
	...
	\psw{$\DS n = \frac{m}{M(\ce{NaCl})} = \SI{0.034}{mol}$}%
}
