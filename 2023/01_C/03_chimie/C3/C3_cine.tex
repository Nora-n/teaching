...
\begin{tcb}*[label=def:ordre](defi)"ror"{Ordre de réaction}
	...
	\begin{itemize}
		...
		\item \psw{
			      $\alpha$ et $\beta$ (ou $m_i$) sont les \textbf{ordres partiels} de
			      la réaction~;
		      }
		\item \psw{
			      $\alpha + \beta$ (ou $m = \sum m_i$) est \textbf{l'ordre global}
			      de la réaction.
		      }
	\end{itemize}
...
\psw{
	\begin{gather*}
		v =
		k \left( a (c_0 - x)\right)^{\alpha}\left( b (c_0 - x)\right)^{\beta}
		\\\Lra
		\boxed{
		v =
