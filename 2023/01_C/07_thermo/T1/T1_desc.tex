...
\begin{tcb}(odgr)<lftt>{Molécules de quelques systèmes}
	...
	\begin{enumerate}
		...
		\item \psw{%
			      Surface $S = \SI{80}{m^2}$ et hauteur $h = \SI{3.0}{m}$ donc volume $V =
				      \SI{240}{m^3}$~; avec $\th = \SI{20}{\degreeCelsius} \Lra T =
				      \SI{293}{K}$ et $P = \SI{1}{bar} = \SI{e5}{Pa}$, soit avec la loi du gaz
			      parfait
			      \[
				      n = \frac{PV}{RT} \approx \SI{e4}{mol}
				      \qso
				      N = n \Nc_A = \SI{6.0e27}{molécules}
			      \]
		      }%
	\end{enumerate}
\end{tcb}
...
\begin{tcb*}(prop){Équilibre thermique}
	Deux systèmes en équilibre thermique l'un avec l'autre n'\textbf{échangent pas
		de chaleur}~; ainsi,
	\begin{center}
		\psw{Deux systèmes en équilibre thermique ont même température~: $\boxed{T_1
					= T\ind{ext}}$}
	\end{center}
\end{tcb*}
...
\begin{tcb*}[breakable](defi){Gaz parfait monoatomique}
	...
	\begin{itemize}
		\item \psw{%
			      Il n'est constitué que de molécules à \textbf{un seul atome}~;
		      }%
		\item \psw{%
