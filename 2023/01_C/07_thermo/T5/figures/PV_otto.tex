...
\begin{tikzpicture}
	...
	\node[myred,above right=.4 and -.00, scale=0.75, rotate=90] at (D) {explosion};
	...
	% POINTS
	\fill[mydarkblue]
	(A) circle(0.05) node[above] {D}
	(B) circle(0.05) node[above] {E}
	(C) circle(0.05) node[below right] {B}
	(D) circle(0.05) node[left] {C}
