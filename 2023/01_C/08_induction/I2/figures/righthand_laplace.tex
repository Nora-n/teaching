...
%    (O) --++ (85:3.4)
%    node[above,scale=1.5] {${\color{myblue}\vv{a}} \times {\color{myred}\vv{b}}$};
%  \draw[thick vector,myblue]
%    (O) --++ (145:3.7) coordinate (A)
%    node[above=2,left=-7,scale=1.5] {$\vv{a}$};
%  \draw[thick vector,myred]
%    (O) --++ (172:3.7) coordinate (B)
%    node[above=4,left=-5,scale=1.5] {$\vv{b}$};
...
%   \draw[force]
%     (O) --++ (82:3.2)
%     node[above,scale=1.5] {$\vv{F} \color{black} = q {\color{veccol}\vv{v}} \times {\color{Bcol}\vv{B}}$};
%   \draw[velocity]
%     (O) --++ (148:3.3) coordinate (V)
%     node[above=2,left=-6,scale=1.5] {$\vv{v}$};
%   \draw[charge+] (O) circle (\R) node[scale=1.4] {$+$};
%   \draw[BField]
%     (O)++(-172:0.7*\R) --++ (-172:3.25) coordinate (B)
%     node[above=4,left=-6,scale=1.5] {$\vv{B}$};
...
\begin{tikzpicture}
  ...
  \draw[force]
    (O) --++ (85:3.4)
    node[above,scale=1.5] {$\vv{F} \color{black} = i {\color{veccol}\vv{\rm AC}}
    \wedge {\color{Bcol}\vv{B}}$};
  \draw[velocity,very thick]
    (O) --++ (145:3.7) coordinate (V)
    node[pos=.9, above right=-6, scale=1.5] {$\vv{\rm AC}$};
  \draw[BField]
    (O) --++ (172:3.7) coordinate (B)
    node[above=4,left=-5,scale=1.5] {$\vv{B}$};
  \draw pic[->,"\huge$\theta$",draw=black,thick,angle radius=35,angle eccentricity=1.24] {angle = V--O--B};
...
%   \draw[vector]
%     (O)++(125:0.77) --++ (78:2.7)
%     node[midway,above=10,left=-3,scale=1.5] {$\vv*{\omega}$};
...
%   \draw[vector]
%     (O)++(125:0.77) --++ (78:2.7)
%     node[midway,above=10,left=-3,scale=1.5] {$\vv{L} = I\vv*{\omega}$};
%   \draw[->,very thick,rotate=-15]
%     (O)++(-250:{\Rx} and {\Ry}) arc (-250:-80:{\Rx} and {\Ry})
%     node[below=-1,scale=1.5] {$\omega$};
%   
...
%     (O)++(106:1.2) --++ (78:2.1)
%     node[midway,left,scale=1.5] {$\vv{I}$};
%   \draw[BField,rotate=-15]
%     (O)++(-250:{\Rx} and {\Ry}) arc (-250:-80:{\Rx} and {\Ry})
%     node[below=1,scale=1.5] {$\vv{B}$};
%   
% \end{tikzpicture}
