...
\enonce{%
	...
	\begin{minipage}{0.64\linewidth}
		...
		masse $m$ est lâchée d'un angle $\th_0$ sans vitesse initiale. On prend $g =
			\SI{9,8}{m.s^{-2}}$.
	\end{minipage}
	\hfill
...
\QR{%
	En prenant l'origine des énergies potentielles en $\th = 0$,
	montrer que l'énergie potentielle totale du système peut s'écrire
	sous la forme~:
	\[
		\Ec_{p,p} = mg\ell (1-\cos\th)
	\]
}{%
	...
	\[
		\Ec_{p,p} = mgz(\th)
	\]
	Or, par rapport à la position $\th = 0$ où le pendule est à la distance $\ell$
	du point O, à un angle $\th$ quelconque la masse est à la distance $\ell\cos(\theta)$. On trouve donc
	\[
		z(\th) = \ell(1-\cos(\theta))
		\Ra
...
\enonce{%
	\begin{tcb}(expe)<itc>{Visualisation des angles,
		...
		\begin{enumerate}
			...
			\item Créer les variables \texttt{deriv\_angle} (dérivée première) et
			      \texttt{dderiv\_angle} (dérivée seconde), en utilisant les
			      fonctions traitements $\rightarrow$ calculs spécifiques
			      $\rightarrow$ dérivée et dérivée seconde.
			      % \item Créer les variables \texttt{deriv\_Pendule} (dérivée première) et
			      %          \texttt{dderiv\_Pendule} (dérivée seconde), en utilisant les
			      %          fonctions traitements $\rightarrow$ calculs spécifiques
			      %          $\rightarrow$ dérivée et dérivée seconde.
			      % \item À partir des variables \texttt{deriv\_Pendule} et
			      %          \texttt{dderiv\_Pendule} (exprimées en degrés), introduire les
			      %          variables \texttt{deriv\_angle} et \texttt{dderiv\_angle} exprimées
			      %          en radians.
			\item Afficher simultanément les trois courbes obtenues, en mettant la
			      fonction \texttt{angle} sur l'axe de droite et les lisser en
...
\QR{%
	...
	\[
		\th(t) = \th_0 \cos(\w_0t)
		\Ra
		\tp(t) = \w_0\th_0 \cos(\w_0t-\frac{\pi}{2})
	\]
...
\QR{%
	...
	Pour plusieurs valeurs de $\th_0$, on mesure la période $T$ observée grâce aux
...
\QR{%
	...
	période $T\ind{simu}$ en fonction de l'amplitude initiale $\th_0$.
}{%
	Idem.
}
...
\QR{%
	...
	précédemment ($T\ind{exp} = f(\th_0)$). Enregistrer votre travail sur
