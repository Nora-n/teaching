...
\QR{%
	...
	écrire le résultat du mesurage de $c_0$ sous la forme $M(c_0) = m(c_0)
		\pm u(c_0)$. Vous justifierez les incertitudes de type B que vous avez
	choisi pour $V_{B, \eqi}$ et $V_A$.
}{%
	Pour le volume versé, on lit sur la pipette de classe A une tolérance de
	\SI{0.02}{mL}. Pour le volume à l'équivalence, on a pris \SI{0.2}{mL} de
	manière grossière~: il faudrait faire une incertitude des deux Monte-Carlo et
	déterminer numériquement l'intersection plutot qu'à la souris pour avoir une
	estimation plus réaliste. On obtient finalement
	\[
		\xul{c_0 = \SI{1.500\pm0.019}{mol.L^{-1}}}
	\]
}%
...
\QR{%
	...
	\[
		u(d) = \frac{M}{\rho_V}*u(c_0)
		\qet
		\boxed{E_n = \frac{\abs{d - d\ind{theo}}}{u(d)}}
		\Ra
		\xul{E_n = \num{8.75}}
	\]
	La mesure est ainsi très éloignée du degré annoncé. Pour du vinaigre qu'on
	achète au supermarché, ça n'est pas grave, surtout s'il sert à nettoyer. Il
	faudrait cependant être critique pour une utilisation plus quantitative,
	notamment pour faire des produits ménagers.
}%
