...
\enonce{%
	\begin{tcb}*(ror)<ctc>{\iconhow~Savoir-faire}
		\begin{itemize}
	...
	\begin{minipage}[t]{.70\linewidth}
		...
		\ce{H2Asc}. Antiscorbutique et anti-infectieux, l'acide ascorbique joue de plus
...
\QR{%
	...
	\textbf{Déterminer} l'expression de sa constante d'équilibre et la calculer.
	...
	\begin{gather*}
		...
		\tag*{$\mathllap{K_2^\circ = \frac{[\ce{I^-}]\ind{eq}^2 [\ce{H^+}]\ind{eq}^2
	...
	\begin{gather*}
		...
		\quad \textbf{totale}
	\end{gather*}
	La réaction est donc thermodynamiquement favorisée et même supposée totale, donc
	elle est adaptée à un titrage direct pour ce point. En revanche, il est dit dans
	l'énoncé qu'elle est \textbf{lente}~: «~de l'ordre de quelques minutes par
	équilibre~». On ne pourrait pas réaliser un dosage colorimétrique précis en
	attendant $15 \times 2$ minutes (voire plus). Il est préférable
	\textbf{d'attendre une unique fois} que la réaction se fasse avec l'excès connu,
	puis de titrer l'excès rapidement et précisément ensuite.
	\smallbreak
...
\QR{%
	...
	incertitude de type A pour obtenir sa valeur expérimentale. Comparer à la
...
\subsection{Dosage en retour}
\enonce{%
	Vous disposez sur votre paillasse d'une solution $S$ obtenue en ayant dissout
	\textbf{un} comprimé de vitamine C dans \SI{500}{mL} d'eau distillée.
}%
...
\QR{%
	...
	utilisée, et estimer l'incertitude sur le volume équivalent.
}{%
	solu
}%
...
\QR{%
	...
	\begin{gather*}
		...
		\boxed{n_0(\ce{H_2Asc}) = n\ind{réagi}(\ce{I_2}) = c_1V_1 -
			\frac{c_2V\ind{eqv,2}}{2}}
		\Ra
		\xul{n_0(\ce{H_2Asc}) = \SI{2.9e-5}{mol}}
	\end{gather*}
}%
\QR{%
	À l'aide d'une propagation des incertitudes par méthode \textsc{Monte-Carlo},
	estimer l'incertitude sur cette valeur (cf.\
	\url{https://capytale2.ac-paris.fr/web/c/c31b-3326128})
}{%
	% TODO: Monte-Carlo sur Capytale
}%
...
\QR{%
	...
	\begin{gather*}
		\xul{n\ind{comp}(\ce{H_2Asc}) = \SI{2.9e-3}{mol}}
		\qet
		\boxed{m\ind{comp}(\ce{H_2Asc}) = n\ind{comp}(\ce{H_2Asc})M(\ce{H_2Asc})}
		\\\beforetext{d'où}
		\xul{m\ind{comp, expe}(\ce{H_2Asc}) = \SI{510.96\pm10}{g.mol^{-1}}}
		\\\beforetext{or}
		\boxed{
			E_n =
