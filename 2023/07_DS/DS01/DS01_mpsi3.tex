...
\exercice[32]{Étude de quelques lentilles minces \ifprof{\hfill \small D'après IESSA 2022 \hfill}}
...
\switch{
	...
	\begin{enumerate}
    \nitem{3} Les questions 1 et 2 montrent que pour un objet réel, une lentille
    ...
    \nitem{5} Nous avons vu qu'avec la relation de \textsc{Descartes}, en isolant
		      $\OAp$ on obtenait
		      \[
			      \OAp = \frac{\OA f'}{\OA + f'}
		      \]
...
\switch{
	...
	\begin{enumerate}
    \nitem{4} De même que précédemment, le calcul de $\OAp$ avec un objet réel ($\OA
    ...
    \nitem{2} Cette fois, si $\OA > 0$ et $f' < 0$, alors $\OAp$ peut changer de
...
\newpage
\exercice[48]{Instruments d'optique à l'infini \switch{\hfill -- Deux parties indépendantes -- \hfill}{\hfill \small D'après CCP PC 2015 \hfill}}
...
\switch{
	...
	\begin{enumerate}
    \nitem{4} L'objet étant à l'infini, l'image intermédiaire  $\ABb$ est située
    ...
    \nitem{3} La relation de conjugaison de \textsc{Descartes} appliquée à la
    ...
    \nitem{3} On obtient la figure~\ref{fig:teleobj} en annexe.
    \nitem{4} D'après la formule de conjugaison de \textsc{Newton} appliquée à la
      lentille $\Lc_3$~:
        \[
    ...
    \begin{itemize}
        \item La taille de l'image intermédiaire est donnée par la relation
              ...
              $\rm O_1F'_1B_1$).
        \item La taille de l'image définitive est donnée par la relation $\ABp =
              \gamma_3 \ABb = \gamma_3 \alpha f'_1$.
        \item $\gamma_3$ est le grandissement introduit par la lentille $\Lc_3$
              dont l'expression est donnée par la formule de \textsc{Newton}~:
              \[
                \gamma_3 = \frac{f'_3}{\obar{\rm F_3F'_1}} = -
                \frac{f'_3}{\obar{\rm F'_1F_3}}
              \]
      \end{itemize}
      \leftcentersright{On en déduit}{$\boxed{\ABp = - \frac{f'_3}{\obar{\rm
      F'_1F3}} \alpha f'_1}$}{$\AN\xul{\ABp = \SI{-4.3}{cm}}$}
	\end{enumerate}
}
...
\switch{
	...
	\begin{enumerate}
		\item Compléter la figure \ref{fig:lunette2} en annexe et représenter le
		      devenir d'un tel faisceau lumineux limité par la monture de lentille
		      objectif (encore appelée diaphragme d'ouverture).
	...
	\ref{fig:lunette3}).
	\begin{enumerate}
		\item Compléter la figure~\ref{fig:lunette3} en annexe. On dit de la monture
		      de l'oculaire qu'elle est le diaphragme de champ de la lunette.
	...
	\begin{enumerate}
    \nitem{6} \xul{L'observation à l'infini} se fait sans fatigue
    ...
    \nitem{6} Les rayons traversant un système optique centré dans les
    conditions de \textsc{Gauss} sont \xul{paraxiaux}, c'est-à-dire peu écartés
    ...
    \nitem{5} Le rayon incident forme un angle $\tt \ll \SI{1}{\radian}$ avec
          ...
          \tt$ et $\tan{(\tt')} = \tt'$.
		      ...
		      \end{gather*}
    \nitem{2} Sur la Figure~\ref{fig:lunette2_corr}, le faisceau incident
    ...
    \nitem{2} On peut appliquer le théorème de \textsc{Thalès} dans les
    triangles $\rm MO_1F'_1$ et $\rm F_2O_2Q$, ainsi~:
            ...
            \]
    \nitem{2} On calcule~: $\xul{D = \SI{2.0}{mm}}$. Comme $D < D_2$, c'est bien le
          diamètre $D_1$ de l'objectif qui limite le diamètre du faisceau
          émergent.

    \nitem{4} Sur la Figure~\ref{fig:lunette3_corr}, on représente le chemin des
    ...
    \nitem{2} L'aberration chromatique d'une lentille provient du fait que
...
\prblm[30]{Étude de pierres précieuses \ifprof{\hfill \small D'après CCP 2007 PC\hfill}}
\restartlist{enumerate}
...
\switch{
	\begin{enumerate}
		\item On considère un milieu 1 d'indice $n_1$ et un milieu
	...
	\begin{enumerate}
    \nitem{8} Comme $n_2<n_1$, le rayon réfracté est plus éloigné de la normale au
		      ...
		      \begin{figure}[htbp!]
			      ...
			      \label{fig:reflim}
		      \end{figure}
    \nitem{2} Le spectre de la lumière blanche est «~étalé~» par le verre~: on met
		      en évidence le fait que le verre est un \xul{milieu dispersif.}
    \nitem{4} La zone 1 est éclairée par les rayons d'angle d'incidence inférieur à
		      ...
		      \xul{La zone 1 est donc la zone sombre et la zone 2 est la zone
			      claire.}
    \nitem{1} L'angle d'incidence qui marque la limite entre les deux zones est
		      l'angle limite de réfraction. On utilise donc le résultat de la
		      question~1 avec $n_1=n_v=\num{1.96}$ et $n_2=n_g=\num{1.55}$~:
		      \[
			      \xul{i_\ell=\ang{52.3}}
		      \]
    \nitem{3} Pour la moissanite, $n_g>n_v$ il n'y a donc plus de réflexion totale
...
\switch{
	...
	\begin{enumerate}
    \nitem{2} La moissanite a une masse volumique plus faible que celle du liquide
...
\switch{
	\begin{enumerate}
		\item Sur la figure \ref{fig:gemme_e1} de l'annexe, tracer l'allure du
          prolongement des rayons réfractés issus de $\mathrm{A}, \mathrm{B},
          \mathrm{C}$ et $\mathrm{D}$, jusqu'à l'écran, dans le cas où l'indice
          de réfraction de la gemme $n_g$ est supérieur à $n_{\rm liq}$. Faire
          de même sur la figure \ref{fig:gemme_e2} dans le cas où l'indice de
          réfraction $n_g$ est inférieur à $n_{\rm liq}$. On ne tiendra pas
	...
	\begin{enumerate}
    \nitem{6} On obtient les Figures~\ref{fig:gemmes_corr} proposées en annexe
    en se rappelant qu'en entrant dans un milieu plus réfringent que le milieu
    incident, les rayons se rapprochent de la normale (et inversement si le
    milieu est moins réfringent).
    \nitem{4} On constate que, dans le cas $n_g>n_{\rm liq}$, la lumière se
		      ...
		      \smallbreak
          On en déduit que la pierre \xul{numéro 1} correspond au cas où
          $n_g<n_{\rm liq}$~: c'est donc le \xul{verre flint ($\num{1,64} <
          \num{1,75}$).}
		      \smallbreak
          Inversement la pierre \xul{numéro 2} correspond au cas où $n_g>n_{\rm
			      liq}$~: c'est donc le \xul{zircon ($\num{1,95} > \num{1,75}$).}
	\end{enumerate}
}
...
\switch{
	...
	\begin{enumerate}
    \nitem{3} Dans l'approximation de \textsc{Gauss}, on ne considère que les rayons
		      paraxiaux, c'est-à-dire les rayons proches de l'axe optique et
		      faiblement inclinés par rapport à l'axe optique.
    \nitem{2} Voir annexe, Figure~\ref{fig:gob1_corr}.
    \nitem{2} Voir annexe, Figure~\ref{fig:gob2_corr}.
    \nitem{4} Soient H\bp et H les points tels que SOH\bp et SOH soient rectangles
        ...
        \[
          ...
          \end{WithArrows}
        \]
        \leftcenters{On en déduit}{$\xul{\tau_1=\num{6e-3}}$}
	\end{enumerate}
}
...
\ifstudent{
	...
	lentille $\Lc_2$ est désormais de \SI{36}{mm} (figure~\ref{fig:montage2}). }
...
\switch{
	...
	\begin{enumerate}
    \nitem{7} On note A$_2$, l'image de A à travers la lentille. D'après la
		      relation de conjugaison de \textsc{Descartes}
		      \[
			      \frac{1}{\obar{\rm SA_2}}-\frac{1}{\obar{\rm SA}} = \frac{1}{f'}
			      \quad \Lra \quad
			      \boxed{\obar{\rm SA_2} =
              \frac{f'\obar{\rm SA}}{f'+\obar{\rm SA}}
            } =
            \xul{\SI{-18}{mm}}
		      \]
		      ...
		      \[
          \boxed{\gamma=\frac{\obar{\rm SA_2}}{\obar{\rm SA}}} = \xul{\num{1.5}}
		      \]
          \leftcenters{On en déduit}{$\boxed{\obar{\rm B_2B'_2}=\gamma \obar{\rm
          BB'}} = \xul{\SI{30}{mm}}$}

    \nitem{3} Sur la Figure~\ref{fig:gob3_corr}, on a bien $\obar{\rm
          SA_2}=\SI{-18}{mm}$. L'image est virtuelle et est \num{1.5} fois plus
          grande.

    \nitem{5} La distance entre l'œil et le plan image $B_2B'_2$ est
		      $\xul{\obar{\rm A_2O}=\SI{218}{mm}}$.

		      Soient C et C$'$ les points extrêmes de l'image $\rm B_2B'_2$ visible
		      depuis O. Les triangles ODD$'$ et OCC$'$ sont semblables, donc
		      d'après \textsc{Thalès}~:
		      \[
			      \boxed{
              \obar{\rm CC'}=
              \frac{\obar{\rm A_2O}}{\obar{\rm SO}}\times \obar{\rm DD'}
            } = \xul{\SI{21,8}{mm}}
		      \]
          On constate que $\obar{\rm CC'} < \obar{\rm B_2B'_2}$, donc on ne voit
          qu'une partie de l'image.
		      \smallbreak
          \leftcenters{On en déduit}{$\boxed{\tau_2=\left (\frac{\obar{\rm
          CC'}}{\obar{\rm B_2B'_2}} \right)^2} = \xul{\num{0.53}}$}
	\end{enumerate}
}
...
\switch{
	...
	\begin{enumerate}
    \nitem{2} Par identification avec la relation de conjugaison de
		      \textsc{Descartes} en posant $S$ le centre optique de la lentille,
		      \[
            \boxed{f'_1=\frac{n_1\obar{\rm CS}}{n_2-n_1}} = \xul{\SI{12}{mm}}
		      \]
    \nitem{2} \leftcenters{De même}{$\boxed{f'_2=\frac{n_1\obar{\rm CS}}{n_2-n_3}}
       = \xul{\SI{35}{mm}}$}
    \nitem{2} Dans ce qui précède, sans liquide, il n'y a que 0,6\% de la surface de
          l'image $\rm B_2B'_2$ visible, tandis qu'avec le liquide il y en a
          53\%. Donc l'image devient visible.
	\end{enumerate}
}
...
\switch{
	...
	\begin{figure}[htbp!]
		...
		\subcaptionbox{Annexe question 3\label{fig:gob2}}
		[.75\linewidth]
		{\includegraphics[width=\linewidth]{P1_2-enon}}
		\subcaptionbox{Annexe question 6\label{fig:gob3}}
		[.75\linewidth]
	...
	\begin{figure}[htbp!]
		\centering
		\subcaptionbox{Téléobjectif\label{fig:teleobj}}
		[.80\linewidth]
		{\includegraphics[width=\linewidth]{teleobj}}
		\subcaptionbox{Rayons de l'infini parallèles\label{fig:lunette2_corr}}
		[.80\linewidth]
		{\includegraphics[width=\linewidth]{lunette2_corr}}
		\subcaptionbox{Rayons de l'infini avec un angle\label{fig:lunette3_corr}}
		[.80\linewidth]
	...
	\begin{figure}[htbp!]
		\centering
		\subcaptionbox{Annexe question 2\label{fig:gob1_corr}}
		[.75\linewidth]
		{\includegraphics[width=\linewidth]{P1_1-corr}}
		\subcaptionbox{Annexe question 3\label{fig:gob2_corr}}
		[.75\linewidth]
		{\includegraphics[width=\linewidth]{P1_2-corr}}
		\subcaptionbox{Annexe question 6\label{fig:gob3_corr}}
		[.75\linewidth]
