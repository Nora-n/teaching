...
notes = np.sort(np.genfromtxt('DS02_note.csv', skip_header=1))
# print(notes)
...
ax.set_xticks(np.arange(min(nmin, 0), max(nmax, 20)+1, step=1.0))
ax.set_yticks(np.arange(0, np.max(_)+1, step=1.0))

ax.set_xlim((min(nmin, 0), max(nmax, 20)))
...
ax.set_title(f'Résultat DS02 : moyenne = {moy:.2f}, '
             + f'médiane = {med:.2f}, '
             + f'écart-type = {std:.2f}',
             fontsize=20)
...
fig = plt.gcf()
fig.savefig(dir_path + '/DS02_rslt.pdf',
            bbox_inches='tight')
