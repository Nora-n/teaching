...
\QR{
	Obtenir l'équation dont $u_a$ est solution lorsque $t>0$ et la mettre sous la forme
	\begin{equation}
		\label{eq:circuit_1}
...
\QR{
	...
	\begin{equation}
		\label{eq:circuit_2}
		\dv[2]{u_b}{t} + \frac{\w_0}{Q_b} \dv{u}{t} + \w_0^2 u_b = 0
	\end{equation}
...
\QR{
	...
	que la résonance~? Donner une condition sur $Q_a$ pour que cette amplitude
	atteigne la résonance. On ne cherchera \textbf{pas} à déterminer la
	pulsation de résonance. Cette condition est-elle remplie au regard de la
	...
	\begin{DispWithArrows*}
		...
		X_r = 1 - \frac{1}{2Q_a{}^2}
	\end{DispWithArrows*}
	Et on observe qu'il existe une racine réelle uniquement si $\boxed{Q_a \ge
			\frac{1}{\sqrt{2}} = \frac{\sqrt{2}}{2}}$ (sinon, racines complexes).
	\par
...
\QR{
	...
	\]
	puis montrer que cette dernière atteint toujours la résonance, quel que soit
...
\QR{
	...
	il n'y a pas résonance. Pour le circuit B, on observe une résonance autour de
	$x=1$ avec une faible largeur ($\Delta x_c = 1/Q_b \approx 0,063$).
