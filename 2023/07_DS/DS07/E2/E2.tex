...
\begin{document}
\exercice[23]{Oscillations d'un métronome}
...
\QR[2]{%
	...
	\begin{gather*}
		...
		\quad \Lra \quad
		\boxed{\tpp + \frac{Mgl - mgx}{J} \sin\th \stm{=} 0}
	\end{gather*}
}%
...
\QR[4]{%
	...
	\begin{gather*}
		...
		\\\Ra
		\w_0 \stm{=} \sqrt{\frac{Mgl - mgx}{J}}
		\Lra
		T_0 \stm{=} 2\pi \sqrt{\frac{J}{Mgl - mgx}}
		\Lra
		\boxed{T_0 \stm{=} 2\pi \sqrt{\frac{mx^ 2 + \frac{2}{5} MR^ 2 + M l ^ 2}{Mgl - mgx}}}
	\end{gather*}
}%
...
\QR[2]{%
	...
	période du métronome correspondent ce mouvement~? Déterminer alors la distance
	$x$ du curseur correspondant à ce \textit{tempo}.
}{%
...
\QR{%
	...
	minute pour un mouvement \textit{allegro} de \num{140} battements par
	minutes~? Calculer la distance correspondante.
}{%
	Pour augmenter la fréquence, il faut diminuer $T_0$, donc diminuer son
	numérateur et augmenter son dénominateur, ce qui revient dans les 2 cas à
	diminuer $x$. On trouve
	% TODO: Calculer x_2
}%
\end{document}
