...
\QR[3]{%
	...
	pour couvrir toutes les longitudes. \pt{1} Cependant, un train de satellite couvre
	«~deux côtés~» et donc $\lfloor 7/2 \rfloor = 4$ trains suffisent. \pt{1} On
	aboutit ainsi à un total de $7\times 4 = 28 $ satellites. \pt{1}
}%
...
\QR[7]{%
	...
	période de révolution de la terre $T_T \approx \SI{86e3}{s}$. \pt{1}
	\smallbreak
...
On a $\dim F = {\rm M.L.T^{-2}} = \dim \alpha {M.L^2.T^{-2}}$. On en déduit
par identification que $\boxed{\dim \alpha = {\rm L^{-1}}}$.
\smallbreak
...
\QR{%
	Exprimer les distances $r_1 = \norm{\vvr{OM_1}}$ et $r_2 =
		\norm{\vvr{OM_2}}$ en fonction de $r_0$, $l$ et $\th$
}{%
	On a $\vvr{OM_1} = \vvr{OS}+\vvr{SM_1} \Rightarrow r_1 = \sqrt{r_0^2 + l^2
...
\QR{%
	...
	\[
		\Ec_m \approx
		- \frac{GM_TM_S}{r_0}
		\pa{1 +\frac{1}{2} \frac{l}{r_0}^2 (3\cos^2(\th) -1)} +
	...
	\[
		...
		\Ec_c + \Ec_p =
		-\frac{GM_TM_S}{r_0}
		\pa{1 +\frac{1}{2} \frac{l}{r_0}^2 \pa{3 \cos^2(\th)-1}} +
		\frac{1}{2}M_S (l\tp)^2
	\]
