...
dans l'état initial, à la température \(T_0\). Le volume total des deux
...
\begin{enumerate}[resume]
	\item Exprimer \(S_2^{(r)}\) et \(S_2^{(c)}\)
	      \begin{tasks}[label=\protect\fbox{\Alph*}](4)
		      \task~{\footnotesize$\DS S_2^{(r)} = nR\ln(\frac{2T_0}{T_0 + T_1})$}
		      \task~$\DS S_2^{(r)} = 0$
		      \task~{\footnotesize$\DS S_2^{(c)} = nR\ln(\frac{2T_0}{T_0 + T_1})$}
		      \task~$\DS S_2^{(c)} = 0$
	      \end{tasks}
\end{enumerate}
