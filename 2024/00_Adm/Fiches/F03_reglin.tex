...
La régression linéaire\ftn{Il faudrait en toute rigueur parler de régression
  \textit{affine} et non linéaire.} vise à \textbf{tester l'accord entre mesure
...
\begin{tcn}[cnt, bld](prop)"bomb"{Attention~!!}
...
\begin{tcn}[breakable](appl)<lftt>{Application~: quel modèle est validé~?}
  ...
  \begin{center}
    ...
    \captionof{figure}{Exemples de régressions linéaires.}
    \label{fig:reglin}
  \end{center}
  \tcblower
  \begin{enumerate}[label=\Alph*)]
    \item \psw{%
      \textbf{Non validée}~: barres d'erreurs ok, mais données clairement
      courbées.
    }%
    \item \psw{%
      \textbf{Validé}~: barres d'erreurs ok, pas de nette tendance.
    }%
    \item \psw{%
      \textbf{Non validé}~: barres d'erreurs non respectées, et une tendance de
      fonction sinus.
    }%
    \item \psw{%
      \textbf{Validé mais douteux}~: barres d'erreurs ok et pas de tendance
      particulière, mais incertitudes probablement surestimées. Conclusion peu
      fiable.
    }%
  \end{enumerate}
\end{tcn}
...
\psw{%
  \[
    y\tikzmark{yn} = a\tikzmark{an}x\tikzmark{xn}
  \]
  \tikz[remember picture, overlay]
  \draw[-stealth, transform canvas={xshift=-6pt, yshift=-6pt}]
  (pic cs:yn) --++ (-10pt,-10pt)
  node[anchor=north east] {$\sin(i_1)$}
  ;
  \tikz[remember picture, overlay]
  \draw[-stealth, transform canvas={xshift=-5pt, yshift=-6pt}]
  (pic cs:an) --++(-5pt,-10pt)
  node[anchor=north] {$n_2/n_1$}
  ;
  \tikz[remember picture, overlay]
  \draw[-stealth, transform canvas={xshift=5pt, yshift=-6pt}]
  (pic cs:xn) --++(15pt,-10pt)
  node[anchor=north] {$\sin(i_2)$}
  ;
}%
\vspace{15pt}
