...
\begin{enumerate}
	...
	\item[b]{La valeur} de la grandeur~: \psw{
...
\begin{tcbraster}[raster equal height=rows]
	\begin{tcb*}(prop){Opérations}
		...
		\end{enumerate}
	\end{tcb*}
	\begin{tcb}(exem)<lftt>'r'{Opérations}
		\begin{enumerate}
			\item[b]{Dérivation}~:
			\psw{
				\[
...
\begin{tcb*}[label=exem:grandeurs](exem)<lftt>{Grandeurs dérivées}
	\begin{center}
		\begin{tabular}{lcll}
			\toprule
...
\begin{tcb*}[sidebyside, lefthand ratio=.55](exem)<lftt>{Changement d'unité}
	\vspace*{-10pt}
	\psw{
		\begin{gather*}
			\SI{1}{m.s^{-1}} =
...
\begin{tcb*}(exem)<lftt>{Recherche d'unités}
...
\begin{tcb*}(exem)<lftt>{Détecter des erreurs}
...
\begin{tcb*}[breakable](exem)<lftt>{Recherche de loi}
