...
\begin{tcbraster}[raster columns=2, raster equal height=rows]
	\begin{tcb*}[label=def:mir](defi){Miroir plan}
		...
		\begin{center}
			\sswitch{%
				\includegraphics[width=4cm, draft=true]{mir_plan}
			}{%
...
\begin{tcb*}[label=def:lents,
	...
	](defi){Lentilles, minces, convergentes et divergentes}
	...
	\begin{isd}[sidebyside align=top]
		\tcbsubtitle{\fatbox{\textbf{Convergente}}}
		...
		\end{center}
		\tcblower
		\tcbsubtitle{\fatbox{\textbf{Divergente}}}
...
\begin{tcbraster}[raster columns=2, raster equal height=rows]
	...
	\end{tcb*}
	\begin{tcb*}[label=prop:co](prop)'r'{Centre optique}
		\psw{%
	...
	\begin{tcb*}[label=def:fv, raster multicolumn=2](defi){Distance focale image
		...
		\begin{isd}
			...
			convergente, négative pour une lentille divergente. \textbf{Elle se
				note usuellement $f'$}.
			\tcblower
...
\begin{tcbraster}[raster columns=2, raster equal height=rows]
	\begin{tcb*}[label=impo:lent_regles,
			list entry={\lte Règles secondaires}
		](ror){Règles second.}
		...
		\end{enumerate}
	\end{tcb*}
	\begin{tcb*}[label=appl:constru](appl)<lftt>'r'{Cas secondaire}
		\begin{center}
...
\begin{tcbraster}[raster columns=2, raster equal height=rows]
	...
	\begin{tcolorbox}[blankest, raster multicolumn=1, space to=\myspace]
		\begin{tcbraster}[raster columns=1]
			...
			\end{tcn}
			\begin{tcn}[add to natural height=\myspace](tool)'r'{Outils}
				\psw{%
					Relation de \textsc{Descartes}
					\[
						\boxed{ \frac{1}{\OFp} = \frac{1}{\OAp} - \frac{1}{\OA}}
					\]
					et $\OA = -x$, $\OAp = D-x$.
...
\begin{tcbraster}[raster columns=2, raster equal height=rows]
	...
	\begin{tcolorbox}[blankest, raster multicolumn=1, space to=\myspace]
		\begin{tcbraster}[raster columns=1]
			\begin{tcn}[add to natural height=\myspace](tool)'r'{Outil}
