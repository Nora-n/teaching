...
\begin{tcb}[label=impl:intensconv](impl){Signe et sens réel du courant}
	...
	\begin{itemize}
		\item \psw{%
			      si $i > 0$, le sens conventionnel est respecté~;
		      }%
		\item \psw{%
			      si $i < 0$, le sens conventionnel est opposé à l'orientation choisie.
		      }%
	\end{itemize}
\end{tcb}
...
\end{tcb}
\begin{tcb}(appl)<lftt>{Débit et nombre d'électrons}
...
\begin{minipage}[c]{.58\linewidth}
	\begin{center}
		...
		\captionof{figure}{Analogie courant-débit}
	\end{center}
\end{minipage}
\hfill
\begin{minipage}[c]{.40\linewidth}
	\begin{center}
		...
		\captionof{figure}{Analogie hauteur-tension}
	\end{center}
\end{minipage}
...
\begin{tcb}[label=exem:circuits, sidebyside](exem)<lftt>{Circuit et schéma}
	...
	\begin{center}
		\sswitch{%
			\includegraphics[width=.5\linewidth, draft=true]{circuit_simple.pdf}
		}{%
...
\begin{tcb}[label=def:descri, sidebyside, righthand ratio=.4](defi){Décrire un
			circuit}
	\begin{itemize}[leftmargin=50pt]
		\olditem[\textbf{Nœud}]: \psw{%
			point où se rejoignent au moins 3 fils.
		}%
		\olditem[\textbf{Branche}]: \psw{%
			portion du circuit entre deux nœuds voisins.
		}%
		\olditem[\textbf{Maille}]: \psw{%
...
\begin{tcb*}[label=prop:mesure, sidebyside, righthand ratio=.4](prop){Voltmètre
			et ampèremètre}
	\tcbsubtitle{\fatbox{\textbf{Voltmètre}}}
	\begin{itemize}
		\item \psw{%
			      Mesure une \textbf{tension}.
		      }%
		\item \psw{%
			      Un voltmètre se monte en \textbf{parallèle}.
		      }%
		\item \psw{%
			      Pour mesurer la tension $U_{AB}$ il faut placer la borne COM au
			      point B.
		      }%
	\end{itemize}

	\tcbsubtitle{\fatbox{\textbf{Ampèremètre}}}

	\begin{itemize}
		\item \psw{%
			      Mesure une \textbf{intensité}.
		      }%
		\item \psw{%
			      Un ampèremètre se monte en \textbf{série}.
		      }%
		\item \psw{%
			      Le sens du courant affiché par l'ampèremètre est relié au sens de
			      branchement.
		      }%
	\end{itemize}
	\tcblower
...
\begin{tcb*}[label=inte:arqs](inte){ARQS}
	\psw{%
		Dans un fil, les électrons sont mis en mouvement par un champ électrique. La
		théorie électromagnétique nous montre que le champ électrique est une onde
		qui se déplace à la célérité $c \approx
		...
		considérer que le champ électrique est le même le long d'un fil si sa taille
		est beaucoup plus petite que la longueur d'onde $\lambda$.
	}%
\end{tcb*}
\begin{tcb*}(appl)<lftt>{Validité de l'ARQS}
...
\end{tcb*}
\begin{tcb*}[label=def:regimecontvar, sidebyside](defi){Régimes continu et
			variable}
	\tcbsubtitle{\fatbox{Régime continu}}
	Toutes les intensités et les tensions du circuit sont constantes au cours du
	temps.
	\tcblower
	\tcbsubtitle{\fatbox{Régime variable}}
	Au moins une tension ou une intensité du circuit varie au cours du temps.
\end{tcb*}

\subsection{Lois de \textsc{Kirchhoff}}
...
\begin{tcb}[breakable](appl)<lftt>{Lois des nœuds, lois des mailles}
	\noindent
