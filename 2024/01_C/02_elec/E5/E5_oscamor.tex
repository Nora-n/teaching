...
\begin{tcb*}[label=demo:solupseudoper, breakable]
	...
	% TODO: On injecte la forme générique
...
\begin{tcb*}[label=demo:transipseudo](demo){Régime transitoire pseudo-pér.}
	...
	\psw{%
		\begin{DispWithArrows*}[fleqn, mathindent=10pt]
			\cancel{E}\exp \left(-\frac{t_{95}}{\tau} \right) &= \num{0.05}
			\cancel{E}
			\CArrow{$\ln (\cdot)$}
			\\\Lra
			- \frac{t_{95}}{\tau} &= \ln(\num{0.05})
			\Arrow{$\num{0.05} = 1/20$ et $\ln (a^{b}) = b \ln a$}
			\\\Lra
			\frac{t_{95}}{\tau} &= \ln(20)
			\Arrow{On isole, on remplace et $2 \ln 20 \approx 2\pi$}
			\\\Lra
			t_{95} &= \tau \ln (20) = \frac{2\ln(20)}{\w_0} Q \approx \frac{2\pi}{\w_0}Q
			\qed
		\end{DispWithArrows*}
	}%
\end{tcb*}
...
\begin{tcb*}[label=prop:transipseudo, sidebyside]
	...
	% \smallbreak
	% Un calcul précis montre que le régime le plus court est atteint pour $Q
	% 	\approx \num{0.72}$, mais il y a alors un \textbf{dépassement de la consigne}.
	% TODO: Figure dépassement ? Mettre dans remarque ?
\end{tcb*}
...
\begin{tcb*}[label=prop:transicrit, sidebyside]
	...
	\vspace{-15pt}
	\tcblower
	C'est le \textbf{régime le plus court \xul{sans dépassement}}.
\end{tcb*}
		      ...
		      \psw{%
			      \[
				      \begin{array}{ll}
					      ...
					      \textbf{Force rappel}     & \Ff = -k(\ell(t) - \ell_0) \ux \\
	...
	\begin{tcb}[label=demo:emecacons
		...
		\psw{%
			\begin{DispWithArrows*}[fleqn, mathindent=5pt]
				m \dv{v}{t} v(t) + \alpha \dv{x}{t} \dv{x}{t} + kx \dv{x}{t} &= 0
				\Arrow{$f \cdot f'$}
				\\\Lra
				\dv{t} \Big(
				\underbracket[1pt]{\frac{1}{2}m v^2}_{= \Ec_c} +
				\underbracket[1pt]{\frac{1}{2}kx^2}_{= \Ec_{p,\rm el}}
				\Big) &=
				- \alpha v^2
			\end{DispWithArrows*}
		}%
