\documentclass{standalone}
\usepackage{mintikz}

\begin{document}
\begin{tikzpicture}[scale=.5, declare function={
				mmin = -1;
				mmax = 16;
				wdth = 1;
				xoff = 4;
				soff = 7;
			}]
	% pH
	\fill[LimeGreen!10]
	(-.5,0) rectangle (.5,14);
	\fill[red!10]
	(-.5,mmin) rectangle (.5,0);
	\fill[blue!10]
	(-.5,14) rectangle (.5,mmax);
	% Scale
	\draw[very thick, -{Stealth[scale=1.3]}]
	(0,mmin) --
	(0,mmax)
	node[above] {\Large$\pk$};
	\node[draw, thick, rounded corners,
		% fill=red!10
	] at (xoff, mmax)
	{\Large\textbf{Acides}};
	\node[draw, thick, rounded corners,
		% fill=blue!10
	] at (-xoff, mmax)
	{\Large\textbf{Bases}};
	\foreach \x in {0, 5, ..., 14}{
			\draw[thick]
			(-.3,\x) -- (.3,\x);
		}
	\foreach \x in {0, ..., 14}{
			\draw
			(-.15,\x) -- (.15,\x);
		}
	% AB scale
	\draw[-{Stealth[scale=1.2]}, thick]
	(soff,14) --
	(soff,mmin)
	node[pos=.75, above, sloped] {\large\textbf{Force des acides}};
	\draw[-{Stealth[scale=1.2]}, thick]
	(-soff,mmin) --
	(-soff,14)
	node[pos=.75, above, sloped] {\large\textbf{Force des bases}};
	% AB
	\foreach \A/\DA/\B/\DB/\K/\KN [count=\i] in {
			\ce{HNO2}//\ce{NO2-}/draw/3.2/3.2,
			\ce{CH3COOH}/draw/\ce{CH3COO-}//9.2/4.74
		}{
			\node[\DB] (B\i) at (-xoff,\K) {\Large\B};
			\node[\DA] (A\i) at (xoff,\K) {\Large\A};
			\node at (0,\K) {$\times$};
			\node[fill=white, opacity=.8, text opacity=1] at (0,\K)
			{\Large\num{\KN}};
		}
	% water
	\foreach \A/\DA/\B/\DB/\K in {
			\ce{H3O+}//\ce{H2O}/draw/0,
			\ce{H2O}/draw/\ce{HO-}//14
		}{
			\node[\DB] at (-xoff,\K) {\Large\gre{\B}};
			\node[\DA] at (xoff,\K) {\Large\gre{\A}};
			\node at (0,\K) {$\times$};
			\node[fill=white, opacity=.8, text opacity=1] at (0,\K)
			{\Large\gre{\num{\K}}};
		}
	\draw[->, very thick]
	(B1) .. controls (A2.south west) .. ($(A2.south)!0.5!(B2.south)$)
	node[pos=.25, left=.25] {\large Défavorisé}
	.. controls (B2.south east) .. (A1)
	% node[below left=.7 and -.5] {\large Favorisé}
	;
\end{tikzpicture}

\end{document}
