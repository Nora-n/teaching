...
\begin{tcb}[label=def:propsto](defi){Proportions stœchiométriques}
	\psw{%
		...
		stœchiométriques si les \textbf{quantités de matière de tous les réactifs
			s'annulent \xul{si l'avancement était maximal}}.
	}%
\end{tcb}
...
\begin{tcb}[width=\linewidth, breakable](appl)<lftt>{Réaction limitée}
	...
	\psw{%
		On simplifie les calculs en ignorant $c^\circ = \SI{1}{mol.L^{-1}}$. Ainsi
		ce polynôme a deux racines~:
		\begin{align*}
			x_{f,1} = \frac{-K^\circ - \sqrt{K^\circ{}^2+4K^\circ c}}{2}
			 & \qet
			x_{f,2} = \frac{-K^\circ + \sqrt{K^\circ{}^2+4K^\circ c}}{2}
			\\
...
\begin{tcb*}[label=prop:K](prop){Constante d'équilibre et avance\mnt}
	...
	\begin{itemize}
		\item \psw{%
			      ...
			      sens $\rightarrow$, et si $K^\circ > \num{e4}$, alors la réaction
			      est \textbf{totale} (ou quasi-totale) dans le sens $\rightarrow$~;
		      }%
		\item \psw{%
			      si $K^\circ < 1$, la réaction est \textbf{défavorisée} dans le sens
			      $\rightarrow$ (favorisée dans l'autre sens), et si $K^\circ <
				      \num{e-4}$, alors la réaction est \textbf{peu avancée} dans le sens
			      $\rightarrow$.
		      }%
	\end{itemize}
	\begin{center}
		\fatbox{\textbf{Il faut savoir repérer les situations d'avancement (quasi)total}}
	\end{center}
\end{tcb*}
...
\begin{tcb}(exem)<lftt>{Sens d'évolution~: précipitation}
	...
	\begin{enumerate}
		\item Si $[\ce{Ag^{+}}]_i = [\ce{Cl-}]_i = \SI{e-3}{mol.L^{-1}}$, alors
		      ...
		      et la réaction se passe dans le sens \psw{direct~: on forme du
			      précipité.}
		\item Si $[\ce{Ag^{+}}]_i = [\ce{Cl-}]_i = \SI{e-6}{mol.L^{-1}}$, alors
		      ...
		      et la réaction se passe dans le sens \psw{indirect~: on dissout le
			      précipité.}
	\end{enumerate}
\end{tcb}
