...
\tikzstyle{thick vector}=[->,line width=4,veccol]
...
\begin{tikzpicture}
	...
	\def\scl{3}
	\draw[thick vector]
	(O) --++ (148:3.3) coordinate (X) node[above=12,left=-12,scale=\scl]
		{$\vec{u}_y$};
	\draw[thick vector,<->]
	(O) ++ (-172:3.25) coordinate (Y) node[above=5,left=-6,scale=\scl]
		{$\vec{u}_z$} --
	(O) --++ (82:3.2) node[above=-6,scale=\scl]
		{$\vec{u}_x$}
	coordinate(Z);
	\draw
	pic[draw, ultra thick, angle radius=25, angle eccentricity=1.3]
		{right angle = X--O--Y};
	\draw
	pic[draw, ultra thick, angle radius=20, angle eccentricity=1.3]
		{right angle = X--O--Z};
	\draw
	pic[draw, ultra thick, angle radius=15, angle eccentricity=1.3]
		{right angle = Y--O--Z};
\end{tikzpicture}
\end{document}
