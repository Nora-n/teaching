...
% 	\frac{\dd{\th}}{\cancel{\dt}}\ut\right)\times\cancel{\dt}
% \end{gather*}
...
\begin{tcb*}[sidebyside, righthand ratio=.25](demo)
	...
	\begin{center}
		La distance ainsi obtenue est \psw{$r\dd{\th}$ sur $\ut$.}
	\end{center}
	\tcblower
	\begin{center}
		\sswitch{
...
\begin{tcb*}(demo)<lftt>{Vitesse en polaires par dérivation}
	...
	\begin{DispWithArrows*}[format=LrL, xoffset=1.5cm]
		...
		\psw{\dv{\ur}{t}}
		& =
		\psw{\dv{\cos(\th)}{t}\ux + \dv{\sin(\th)}{t}\uy}
...
\begin{tcb*}[breakable](rema){Dérivée composée en physique}
	...
	\psw{
		\begin{DispWithArrows*}
			\dv{t}\/(\cos(\theta)) &= \dv{t} \dv{\th}{\th}\/ (\cos(\theta))
			\Arrow{On réarrange}
			\\\Lra
			\dv{t}\/(\cos(\theta)) &= \dv{\th}{t} \dv{\th} (\cos(\theta))
...
\begin{tcb*}*(demo)<lftt>"ror"{Accélération en polaires}
	...
	\begin{DispWithArrows*}[format=LrL, xoffset=1.5cm]
		...
		\psw{\ut}
		& =
		\psw{-\sin(\th)\ux + \cos(\th)\uy}
		\CArrow{$\dv{t}\/(\cdot)$}
		\\\Lra
		& \qquad
		\psw{\dv{\ut}{t}}
		& =
		\psw{\dv{(-\sin(\th))}{t}\ux + \dv{\cos(\th)}{t}\uy}
		\Arrow{Dérivée composée}
		\\\Lra
		& \qquad
		\psw{\dv{\ut}{t}}
		& =
		\psw{-\tp\cos(\th)\ux - \tp\sin(\th)\uy}
		\Arrow{Factorisation}
		\\\Lra
		& \qquad
		\psw{\dv{\ut}{t}}
...
\psw{
	\begin{gather*}
		\vf
		= \frac{\OM(t+\dt)-\OM(t)}{\dd{t}}
		= \frac{\OM(t+\dt)+\vv{\Mr(t)\Or}}{\dd{t}}
		= \frac{\vv{\Mr(t)\Mr(t+\dt)}}{\dd{t}}
	\end{gather*}
}
