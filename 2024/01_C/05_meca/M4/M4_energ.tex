	...
	\begin{isd}[sidebyside align=top]
		...
		\psw{
			\begin{align*}
				...
				\\\Lra
				\Aboxed{\dv{\Ec_c\MRg}{t} & = \sum_i \Pc\Rg(\Ff_i)}
				\qed
			\end{align*}
		}
		\vspace{-15pt}
...
\begin{itemize}
	...
	\item Si le degré de liberté $x$ est un angle $\th$, on aura
	      \psw{$\DS \de W = F_\th r\dd{\th} \Ra F_\th = -\frac{1}{r}\dv{\Ec_p}{\th}$.}
\end{itemize}
...
\begin{DispWithArrows*}
	\psw{F(x)} &= \psw{F(x_{\eql}) + (x-x_{\eql})\times \qty(\dv{F}{x})_{x_{\eql}}}
	\Arrow{$F = -\dv{\Ec_p}{x}$}
	\\\Lra
	\psw{F(x)} &=
	\psw{
	- \qty(\dv{\Ec_p}{x})_{x_{\eql}} -
	(x-x_{\eql})\times\qty(\dv[2]{\Ec_p}{x})_{x_{\eql}}
	}
	\Arrow{$F(x_{\eql}) = 0 = \qty(\dv{\Ec_p}{x})_{x_{\eql}}$}
	\\\Lra
	\psw{F(x)} &= \psw{-(x-x_{\eql}) \qty(\dv[2]{\Ec_p}{x})_{x_{\eql}}}
\end{DispWithArrows*}
...
\begin{tcb*}(appl)<lftt>'l'{Équilibre d'un ressort par énergie potentielle}
	...
	\begin{minipage}{0.70\linewidth}
		\psw{%
			...
			\]
			Ainsi, la position d'équilibre est en $x_{\eql} = \ell_0$, et elle est
			stable.
		}%
		\vspace{-15pt}
	\end{minipage}
...
\psw{%
	\begin{gather*}
		\Ec_p(x) = \Ec_p(x_{\eql}) + (x-x\ind{eq})
		\underbracket[1pt]{\qty(\pdv{\Ec_p}{x})_{x_{\eql}}}_{=0} +
		\frac{(x-x\ind{eq})^2}{2}
		\underbracket[1pt]{\qty(\pdv[2]{\Ec_p}{x})_{x_{\eql}}}_{k}
		\\\Ra
...
\psw{%
	\begin{gather*}
		\frac{1}{2}m(2\xpp\xp) + \frac{1}{2}k 2\xp(x-x_{\eql}) = 0
		\Lra
		\boxed{\xpp + \frac{k}{m}(x-x_{\eql}) = 0}
	\end{gather*}
}%
...
\begin{itemize}
	% \subsubsection{Cas $\protect\Ec_m < 2mg \protect\ell$}
	\item[b]{Cas $\Ec_m < 2mg\ell$}
	      ...
	      \begin{minipage}{0.45\linewidth}
		      \begin{center}
			      ...
			      \captionof{figure}{État initial $\th = \th_0$~: toute l'énergie est
				      sous forme d'énergie potentielle, et l'énergie est nulle.}
		      \end{center}
	      \end{minipage}
	      \hfill
	      \begin{minipage}{0.45\linewidth}
		      \begin{center}
			      ...
			      \captionof{figure}{La masse perd de l'énergie potentielle tout en
				      restant sur la courbe. L'énergie mécanique étant conservée, elle
				      gagne de l'énergie cinétique et donc de la vitesse.}
		      \end{center}
	      \end{minipage}
	      ...
	      \begin{minipage}{0.45\linewidth}
		      \begin{center}
			      ...
			      \captionof{figure}{La masse a perdu toute son énergie potentielle~:
				      son énergie cinétique et donc sa vitesse sont maximales.}
		      \end{center}
	      \end{minipage}
	      \hfill
	      \begin{minipage}{0.45\linewidth}
		      \begin{center}
			      ...
			      \captionof{figure}{La masse a perdu toute son énergie cinétique et a
				      ...
				      $-\th_0$, et repart dans l'autre sens.}
		      \end{center}
	      \end{minipage}
	...
	\item[b]{Cas $\Ec_m > 2mg\ell$}
	      \smallbreak
	      Si l'énergie mécanique totale est supérieure à l'énergie potentielle
	      maximale, ça veut dire qu'il y a un excédent d'énergie cinétique. Ainsi,
	      le pendule tourne autour de l'axe de rotation, et lorsqu'il arrive en
	      $\th = \pi$ l'énergie potentielle est maximale mais l'énergie cinétique
	      n'est pas nulle~: il continue sa rotation, sans oscillation. La vitesse
	      n'est cependant pas constante, elle est plus faible en haut qu'en bas du
	      mouvement (par conservation de l'énergie mécanique).
