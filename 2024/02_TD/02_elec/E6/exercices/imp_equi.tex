...
\QR{%
	...
	\begin{center}
		\includegraphics[scale=1]{imp_equi_a_plain}
	\end{center}
}{%
	On commence par convertir le circuit avec les impédances complexes~:
	\begin{tasks}[label=\bdmd](3)
		\task $\Zu_{C_1} = \frac{1}{\jj C_1\w}$~;
		\task $\Zu_L = \jj L\w$~;
		\task $\Zu_{C_2} = \frac{1}{\jj C_2\w}$.
	\end{tasks}
	\begin{center}
		\includegraphics[scale=1]{imp_equi_a}
	\end{center}
	On peut ensuite déterminer l'impédance équivalente à l'association en
	parallèle de $L$ et $C_2$. Avec les admittances, on a
	\begin{gather*}
		\Yu\ind{eq,1} = \Yu_{C_2} + \Yu_{L}
		\Lra
		\Zu\ind{eq,1}
...
\QR{%
	\leavevmode\vspace*{-25pt}\relax
	\begin{center}
		\includegraphics[scale=1]{imp_equi_b_plain}
	\end{center}
}{%
	Ici, on utilise que $\Zu_R = R$ et comme précédemment, on effectue
	...
	équivalente~:
	\begin{center}
		\includegraphics[scale=1]{imp_equi_b}
	\end{center}
	\begin{gather*}
		\Zu\ind{eq, 1}
