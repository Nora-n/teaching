...
\QR{%
	...
	\begin{minipage}{0.50\linewidth}
		\begin{itemize}
			...
			\item[b]{Position de la masse} : $\OM = x(t)\ux$~;
			\item[b]{Longueur ressort 1} : $\ell_1(t) = x(t) - x_0(t)$~;
			\item[b]{Longueur ressort 2} : $\ell_2(t) = L-x(t)$.
		\end{itemize}
	\end{minipage}
	\hfill
	\begin{minipage}{0.50\linewidth}
		...
		\begin{enumerate}
			...
			\item Rappel du ressort 1
			      $\vv*{F}{1} = -k_1(\ell_1(t) - \ell_{10})\ux$~;
			\item Rappel du ressort 2
			      $\vv*{F}{2} = k_2(\ell_2(t) - \ell_{20})\ux$~;
...
\QR{%
	...
	de gauche est immobile en $x_0 = 0$.
}{%
	...
	\begin{gather*}
		...
		\left(
		\begin{array}{c}
				-k_1(\ell_1(t) - \ell_{10}) +k_2(\ell_2(t) - \ell_{20}) -h v \\
				-mg + R
			\end{array}
		\right)
	\end{gather*}
	...
	\begin{gather*}
		\boxed{m \dv[2]{x}{t} + h \dv{x}{t}
			= -k_1(\ell_1(t) - \ell_{10}) +k_2(\ell_2(t) - \ell_{20})}
	\end{gather*}
	...
	\end{gather*}
	À l'équilibre les dérivées de $x(t)$ sont nulles, d'où
	\begin{gather*}
		0 = -k_1(x(t) - x_0(t) - \ell_{10}) + k_2(L-x(t) -\ell_{20})
	\end{gather*}
	Ainsi, avec $x_{0, \eql}(t) = 0$ et $L = \ell_{10}+\ell_{20}$ (d'après
...
\QR{%
	On introduit $X(t) = x(t) - x\ind{eq}$. Établir l'équation différentielle
	sur $X(t)$ lorsque la paroi bouge.
}{%
	Cette fois-ci, on garde $x_0(t)$ dans l'équation. Il vient alors
	\begin{gather*}
		m\ddot{x} + h\dot{x} + (k_1+k_2)(x(t)-x\ind{eq})  = k_1x_0(t)
	\end{gather*}
	et en effectuant le changement de variable $X(t) = x(t)-x\ind{eq}$, on trouve
	l'équation habituelle
	\begin{gather*}
		\boxed{m \ddot{X} + h \dot{X} + kX(t) = KX_{0m}\cos(\wt)}
	\end{gather*}
	avec $k = k_1+k_2$.
}
...
\enonce{%
	...
	complexes $\xul{x}_0(t) = X_{0m} \exp(\jwt)$, $\Xu(t) = X_m \exp(\jj(\wt +
			\f))$ et $\xul{v}(t) = V_m \exp(\jj(\wt + \phi))$ associées à $x_0(t)$, $X(t)$
	et $v(t) = \dot X(t)$.
}
