...
def X(w):
    return K * Im / k * 1 / (1 - (w / w0) ** 2 + 1j * (w / (Q * w0)))
...
ax.set_ylabel("$X_m(\\omega)~[10^{-2}\\mathrm{m}]$", fontsize=15, rotation=0)
...
fig.savefig("./hp_ampl.pdf", bbox_inches="tight")
...
ax.plot(wlist, arg_list, color="limegreen")

fig.savefig("./hp_arg.pdf", bbox_inches="tight")
