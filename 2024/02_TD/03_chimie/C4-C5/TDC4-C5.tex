...
\begin{blocQR}
	\item
	\enonce{%
		Une solution contient initialement des ions \ce{Pb^2+} à la concentration
...
\enonce{%
	...
	\begin{minipage}[c]{.45\linewidth}
		\vspace{0pt}
		\begin{center}
			\includegraphics[width=\linewidth]{vinaigre_titrage-fig1}
		\end{center}
	\end{minipage}
}
\QR{%
	...
	\begin{center}
		\includegraphics[height=3cm]{vinaigre_dilu-fig1}
		\includegraphics[height=6cm]{vinaigre_verrie-fig1}
	\end{center}
}
\QR{%
...
\QR{%
	...
	\begin{center}
		...
		\begin{tabularx}{.8\linewidth}{|l|c||YdYdYdY|}
			...
			\hline
			$\xmathstrut{\rhgt}$
			Équivalence                    & $\xi = \xi_{\eqi}$ &
			$c_1V_1 - \xi\ind{equi}$       & \vline             &
			$cV\ind{equi} - \xi\ind{equi}$ & \vline             &
			$\xi\ind{equi}$                & \vline             &
			excès                                                 \\
			\hline
		\end{tabularx}
	\end{center}
	À l'équivalence, on a introduit $n_{\ce{HO-},\eqi} = cV\ind{equi} =
		\SI{1.275e-3}{mol.L^{-1}}$ d'hydroxyde, et on a consommé en totalité les deux
	produits, soit ici
	\begin{gather*}
		\boxed{n_{\ce{CH_3COOH},\eqi} = n_{\ce{HO-},\eqi}}
		\Lra
...
\QR{%
	...
	Un acide fort a une réaction totale avec l'eau~:
	\begin{center}
		\def\rhgt{0.35}
		\centering
		\begin{tabularx}{\linewidth}{|l|c||YdYdYdY|}
	...
	\begin{gather*}
		\beforetext{Ainsi,}
		[\ce{H_3O^+}]\ind{eq} = 2 c_0
		\Lra
		\boxed{\pH = - \log (2c_0/c^\circ)}
		\Ra
		\xul{\pH = \num{1.7}}
	\end{gather*}
}
...
Seule la première réaction $\ce{H2SO4\aqu{} + H2O\liq{} = HSO4^-\aqu{} +
		H3O^+\aqu}$ est totale, ce qui donne $[\ce{H_3O^+}] = c_0 = [\ce{HSO_4^-\aqu}]$. On part de cet
état pour la seconde~:
\begin{center}
	\def\rhgt{0.35}
	\centering
	\begin{tabularx}{\linewidth}{|l|c||YdYdYdY|}
	...
	\end{tabularx}
\end{center}
On calcule avec $K_A = 10^{\num{-1.9}}$~:
\begin{gather*}
	K_A = \frac{x\ind{eq}(c_0+x\ind{eq})}{(c_0-x\ind{eq})c^\circ}
	\Lra
	x\ind{eq}^2 + x\ind{eq}(c_0 + c^\circ K_1) - c_0 c^\circ K_A = 0
	\\\Ra
	\Delta = (c_0 + c^\circ K_A)^2 + 4K_Ac_0 c^\circ
	\\\Ra
	\boxed{x_{\equ,\pm} = \frac{- (c_0+c^\circ K_A) \pm \sqrt{\Delta}}{2}}
	\\\beforetext{$x_+ > 0 \Ra$}
	\xul{x\ind{eq} = \SI{4.6e-3}{mol.L^{-1}}}
	\\\beforetext{Or,}
	\boxed{\pH = -\log (\frac{c_0+x\ind{eq}}{c^\circ})}
	\Ra
	\xul{\pH = \num{1.8}}
\end{gather*}
Ce qui reste globalement acide fort.
}
...
\QR{%
	...
	\begin{gather*}
		...
		{c^\circ}^2K_s = [\ce{Zn^2+}]\ind{eq}[\ce{CO_3^2-}]\ind{eq}
		\Lra
		{c^\circ}^2K_s = s^2
		\\\Lra
		\boxed{s = c^\circ \sqrt{K_s} }
		\Ra
		\xul{s = \SI{3.98e-6}{mol.L^{-1}}}
	\end{gather*}
}
\QR{%
	...
	\begin{gather*}
		\beforetext{On trouve}
		{c^\circ}^3K_s = [\ce{Zn^2+}]\ind{eq}[\ce{CN^-}]\ind{eq}^2
		\Lra
		{c^\circ}^3K_s = s (2s)^2 = 4s^3
		\\\Lra
		\boxed{s = c^\circ\left( \frac{K_s}{4} \right)^{1/3}}
		\Ra
		\xul{s = \SI{3.97e-5}{mol.L^{-1}}}
	\end{gather*}
}
\QR{%
	...
	\begin{gather*}
		\beforetext{On trouve}
		{c^\circ}^5K_s = [\ce{Zn^2+}]\ind{eq}^3[\ce{PO_4^3-}]\ind{eq}^2
		\Lra
		{c^\circ}^5K_s = (3s)^3 (2s)^2 = 108 s^5
		\\\Lra
		\boxed{s = c^\circ\left( \frac{ K_s}{108} \right)^{1/5}}
		\Ra
		\xul{s = \SI{1.56e-7}{mol.L^{-1}}}
	\end{gather*}
}
...
\QR{%
	...
	La couleur d'une substance est la couleur \textbf{complémentaire} de celel
	qu'elle absrobe le plus. pour \ce{HIn}, la longueur d'onde du maximum
	d'absorption est associée au violet, donc une solution tonenant uniquement
	\ce{HIn} est \xul{jaune}. Réciproquement, la longueur d'onde la plus absorbée
	par \ce{In^-} est rouge, si bien qu'une solution ne contenant que la forme
	basique apparaît \xul{bleue}.
}
\QR{%
	Quelle est la couleur d'une solution de BBT dans sa zone de virage~?
}{%
	Dans la zone de virage, les deux formes acide et basique sont présentes en
	quantités macroscopiques et la couleur de la solution est donnée par les
	règles de la \textbf{synthèse soustractive} (chaque forme absorbe une partie du
	rayonnement)~: elle apparaît \xul{verte}.
}
\QR{%
	...
	L'absrorbance d'une solution contenant $N$ espèces colorées est donnée par
	\[
		A = \sum_{n=1}^{N}\ep_n (\lambda) \ell c_n
	\]
	avec $\ell$ la longueur de la cuve de spectrophotométrie, $\ep_n(\lambda)$ le
	coefficient d'absorption molaire de l'espèce $n$ à la longueur d'onde de
	travail $\lambda$, et $c_n$ la concentration molaire de l'espèce en question.
	Cette loi est valable dans que les solutions sont \textbf{suffisamment diluées}.
}
\enonce{%
...
Soit $\ep_a$ et $\ep_b$ les coefficient d'absorption molaire des formes acide
et basique à la longueur d'onde $\lambda_1$, respectivement. Par définition,
$c = [\ce{HIn}] + [\ce{In^-}]$.
\begin{itemize}
	\item En milieu fortement acide, la forme acide est très majoritaire donc
	      $[\ce{HIn}] \approx c$, soit
	      \[
		      A_1 = \ep_a \ell c
	      \]
	\item En milieu fortement basique, c'est cette fois la forme basique qui est
	      très majoritaire, soit
	      \[
		      A_2 = \ep_b \ell c
	      \]
	\item Pour le pH intermédiaire, les deux formes contribuent à l'absorbance,
	      soit
	      \begin{DispWithArrows*}
		      A_S = \ep_a \ell [\ce{HIn}]_S &+ \ep_b \ell [\ce{In^-}]_S
		      \\\Ra
		      A_1 - A_S = \ep_a \ell (c - [\ce{HIn}]_S) - \ep_b \ell [\ce{In^-}]_S
		      \quad &\text{et} \quad
		      A_S - A_2 = \ep_a \ell [\ce{HIn}]_S + \ep_b \ell ([\ce{In^-}]_S - c)
		      \Arrow{$c = [\ce{HIn}]_S + [\ce{In^-}]_S$}
		      \\\Lra
		      A_1 - A_S = (\ep_a - \ep_b) \ell [\ce{In^-}]_S
		      \quad &\text{et} \quad
		      A_S - A_2 = (\ep_a - \ep_b) \ell [\ce{HIn}]_S
		      \\\Lra
		      \Aboxed{\frac{A_1 - A_S}{A_S - A_2} &= \frac{[\ce{In^-}]_S}{[\ce{HIn}]_S}}
		      \qed
	      \end{DispWithArrows*}
\end{itemize}
}
\QR{%
	...
	Comme le pH de la solution $S$ est connu, le $\pk$ s'en déduit directement
	avec la relation de \textsc{Henderson}~:
	\[
		\pH_S = \pk + \log \frac{[\ce{In^-}]_S}{[\ce{HIn}]_S}
		\qso
		\boxed{\pk = \pH_S - \log \frac{A_1 - A_S}{A_S - A_2}}
		\Ra
		\xul{\pk = \num{7.2}}
	\]
}
...
La forme basique de l'amine est prédominante à pH élevé, alors que la forme
basique prédomine à pH faible. Avec la relation de \textsc{Henderson}, on a
\[
	\pH = \pk + \log \frac{\ce{C_nH_{2n+1}NH2}}{\ce{C_nH_{2n+1}NH_3^+}}
	\Ra
	\pH = \pk
\]
quand les concentrations sont égales. On lit alors $\xul{\pk = \num{5.7}}$.
}
\QR{%
	Écrire l'équation de la réaction. Calculer sa constante d'équilibre et
	justifier qu'elle peut servir de support de titrage.
}{%
	Une réaction de titrage doit être quantitative. Or, ici la réaction s'écrit
	\begin{gather*}
		\ce{C_nH_{2n+1}NH_2\aqu{} + H_3O^+\aqu{} -> C_nH_{2n+1}NH_3^+\aqu{} +
		H_2O\liq{}}
		\\\Ra
		K = \frac{[\ce{C_nH_{2n+1}NH_3^+}]\ind{eq}c^\circ}
		{[\ce{C_nH_{2n+1}NH2}]\ind{eq}[\ce{H_3O^+}]\ind{eq}} = \frac{1}{K_A}
		\Lra
		\xul{K = 10^{\num{5.7}}}
	\end{gather*}
	Ce qui est bien quantitatif~: elle peut \textit{a priori} serivir de titrage
	(il faudrait qu'on s'assure qu'elle soit rapide).
}
\QR{%
	Justifier qualitativement l'allure de la courbe de pH, et en particulier
	l'existence du saut.
}{%
	Initialement, seule la forme basique de l'amine est présente en solution~: il
	est donc normal que le pH soit élevé. On ajoute ensuite de l'acide
	chlorhydrique qui consomme de la base et forme de l'acide, donc le pH diminue,
	et après qu'un large excès d'acide chlorhydrique ait été versé, le pH est
	faible.
	\smallbreak
	Il y a un saut dans la courbe $\pH = f(V)$ au \textbf{voisinage de
		l'équivalence}, puisqu'alors les espèces dominantes changent.
}
\QR{%
	Proposer un indicateur coloré adapté au repérage de l'équivalence.
}{%
	Le meilleur indicateur coloré pour repérer l'équivalence est celui dont la
	zone de virage est située le plus au milieu possible du saut de pH, car c'est
	lui qui donnera le changement de couleur le plus net~: ici, c'est le
	\textbf{bleu de bromothymol}.
}
\QR{%
	Déterminer la formule de l'amine.
}{%
	\centers{\textbf{Compte tenu de la stœchiométrie}}
	\smallbreak
	Lorsque l'équivalence est atteinte, la quantité de matière versée en
	$\ce{H_3O^+}$ est égale à la quantité de matière initiale en amine $n_0$, soit
	\[
		c_A V\ind{eq} = n_0 = \frac{m}{M}
		\Lra
		\boxed{M = \frac{m}{c_A V\ind{eq}}}
	\]
	Or, la masse molaire d'une molécule se calcule en sommant les masses molaires
	de ses atomes constitutifs~:
	\begin{gather*}
		M =
		n M_{\ce{C}} + (2n+3) M_{\ce{H}} + M_{\ce{N}} =
		(M_{\ce{C}} + 2 M_{\ce{H}})n + 3M_{\ce{H}} + M_{\ce{N}}
		\\\Ra
		\boxed{
			n = \frac{1}{M_{\ce{C}} + 2M_{\ce{H}}}
			\left( \frac{m}{c_A V\ind{eq}} - 3M_{\ce{H}} - M_{\ce{N}} \right)
		}
		\Ra
		\xul{n = 4}
		\\\beforetext{Ainsi elle s'écrit}
		\boxed{\ce{C_4H_9NH_2}}
	\end{gather*}
}
