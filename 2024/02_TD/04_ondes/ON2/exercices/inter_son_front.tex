...
\QR{%
	Exprimer le déphasage $\D\f$ au point M entre les ondes issues de HP1 et HP2.
}{%
	À partir de HP1, repéré par le point H$_1$, les ondes parcourent la distance
	$D+x$ pour arriver au micro. À partir de HP2, repéré par le point H$_2$, elles
	parcourent la distance $D-x$. Ainsi,
	\begin{align*}
		\D\f_{1/2}(\Mr)
		 & = -k\D L_{1/2}(\Mr) + \underbrace{\D\f_0(\Mr)}_
		{\mathclap{= 0 \text{ d'après l'énoncé}}}
		\\
		 & = - k \left( {\rm \abs{H_1M} - \abs{H_2M}} \right)
		\\
...
\QR{%
	...
	Les ondes $p_1(\Mr, t)$ et $p_2(\Mr,t)$ étant de même amplitude $P_0$, on a
	que l'onde somme $p(\Mr, t) = p_1(\Mr, t) + p_2(\Mr, t)$ est d'amplitude
	$P(\Mr)$ telle que, après démonstration (cf. cours),
	\[
...
\QR{%
	...
	constructives au point M. En déduire la distance $d$ entre deux maximums
	successifs d'intensité sonore.
}{%
