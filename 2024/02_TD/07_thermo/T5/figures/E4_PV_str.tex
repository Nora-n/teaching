...
\begin{tikzpicture}
	...
	($(D)!.20!(A)$)++(-.2,0) --++ (.5,0)
	node[at start,left,scale=.8] {$Q_{23}$};
	\draw[>={LaTeX[width=5,length=4]},->,line width=2, red, opacity=.5]
	($(A)!.30!(B)$)++(0,-.4) --++ (0,-.5)
	node[at start,above,scale=.8] {$Q_{34}$};

	\draw[>={LaTeX[width=5,length=4]},->,line width=2,blue, opacity=.5]
	($(B)!.72!(C)$)++(-.2,0) --++ (.5,0)
	node[at end, right=.00,scale=.8] {$Q_{41}$};
	\draw[>={LaTeX[width=5,length=4]},->,line width=2,blue, opacity=.5]
	($(D)!.80!(C)$)++(-0.05,0.1) --++ (0,-.5)
	node[at end, below=-0.1, scale=.8] {$Q_{12}$};
