...
\chapter{\cswitch{%
	  ...
	  Images \`a l'infini~: utilisation de la lunette autocollimatrice%
  }%
 }%
...
\enonce{%
	...
	\section{Objectifs}
	\begin{itemize}
		\item Utiliser une lunette autocollimatrice pour \textbf{reconnaître} et
		      \textbf{utiliser} un faisceau parallèle~;
...
\enonce{%
	...
	\begin{minipage}[c]{.48\linewidth}
		...
		oculaire.
		\bigbreak
		Entre l'oculaire et le réticule est disposée une lame
...
\enonce{%
	...
	\begin{tcb}(expe)<itc>{Réglage de l'oculaire}
		\begin{enumerate}
	...
	\begin{tcb}(expe)<itc>{Réglage sur l'infini}
		\begin{enumerate}
...
\enonce{%
	...
	\begin{tcb}(expe)<itc>{Montage pour distance focale}
		\begin{enumerate}
			\item Prendre comme objet $\ABr$, la plaque constituée des lettres F sur
			      ...
			      $\SI{10}{cm}$ ou $\SI{20}{cm}$. Éclairer l'objet $\ABr$ avec la
			      lampe spectrale $(LP)$. Cette lampe doit être allumée une dizaine de
			      minutes avant l'expérience pour chauffer. Interposer un dépoli
			      devant la lampe pour limiter la luminosité et protéger vos yeux.
	...
	\begin{tcn}(inte){Astuce}
		Vérifier que la position de la lunette n'a pas d'importance en la déplaçant
		sur le banc optique, ce qui montre l'existence d'un faisceau de lumière
		parallèle.
	\end{tcn}
}%
...
\QR{%
	Estimer alors la distance focale $f'$ de la lentille, ainsi que son
	incertitude.
}{%
...
\QR{%
	...
	Résultats en cours…
	\smallbreak
	Si vous constatez un désaccord trop grand
...
\enonce{%
	...
	\begin{tcb}[breakable](expe)<itc>{Observation au viseur}
		\begin{enumerate}
...
\QR{%
	Faire la manipulation avec les différentes lentilles convergentes
...
\enonce{%
	\begin{tcb}(expe)<itc>{Théorème des vergences}
		\begin{enumerate}
...
\QR{%
En utilisant la méthode précédente, mesurer la distance focale totale
...
\setlist[blocQR,1]{leftmargin=10pt, label=\clenumi}
\QR<[start=2]>{%
	Quelle régression linéaire doit-on effectuer~?
}{%
	solu
}%
...
\setlist[blocQR,1]{leftmargin=10pt, label=\sqenumi}
\QR<[start=5]>{%
	Réaliser ensuite une régression linéaire \textbf{à la calculatrice} de
...
\QR{%
	Ces valeurs de $a$ et $b$ vous semblent-elles cohérentes~? Justifier.
}{%
	solu
}%

\QR{%
	Reprendre les deux dernières questions avec l'activité
	\texttt{Capytale}.
}{%
	Voir en ligne.
}%
