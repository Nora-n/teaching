...
\QR{%
	...
	\[
		\boxed{v (t) = k [\ce{RCOOR}'](t)[\ce{HO-}](t)}
	\]
}
...
Avec $[\ce{RCOOR}'](t) = [\ce{HO-}](t) = c_0-x (t)$, on a
\[
	\boxed{v = k(c_0-x (t))^{2}}
\]
}
...
\QR{%
	...
	\label{q:reglin}
	\begin{align*}
		v = k (c_0-x(t))^{2}           & = - \dv{(c_0-x)}{t} = \dv{x}{t}
		% \notag
		\\\Lra
		\frac{\dd{x}}{(c_0-x (t))^{2}} & = k \dd{t}
		\label{eq:toint}
		\\\Ra
		\frac{1}{c_0-x}                & = kt + A
		\\\qor
...
\QR{%
	...
	{$\dfrac{\sigma_0-\sigma_\infty}{\sigma(t)-\sigma_\infty} =
			\dfrac{c_0}{c_0-x(t)}$}
}{%
	On calcule~:
	\begin{align*}
		...
		\\\Ra
		\frac{\sigma_0-\sigma_{\infty}}{\sigma(t)-\sigma_{\infty}} & =
		\frac{\dcancel{(\lambda_{\ce{HO-}} - \lambda_{\ce{RCOO}'})}c_0}
		{\dcancel{(\lambda_{\ce{HO-}} - \lambda_{\ce{RCOO}'})}c_0 -
			\dcancel{(\lambda_{\ce{HO-}} - \lambda_{\ce{RCOO}'})}x(t)}
		\\\Lra
		\Aboxed{
		\frac{\sigma_0-\sigma_{\infty}}{\sigma(t)-\sigma_{\infty}} & =
			\frac{c_0}{c_0-x(t)}
...
\QR{%
	...
	doit être vérifiée~:
	\[\dfrac{\sigma_0-\sigma_\infty}{\sigma(t)-\sigma_\infty} = c_0 k t +1\]
}{%
	D'après~\ref{q:reglin},
	\begin{DispWithArrows*}[]
		\frac{1}{c_0-x(t)} &= kt + \frac{1}{c_0}
		\CArrow{$\times c_0$}
		\\\Lra
		\frac{c_0}{c_0-x(t)} &=
		c_0kt+1
...
\QR{%
	...
	On trace donc $\frac{\sigma_0 - \sigma_{\infty}}{\sigma(t) - \sigma_{\infty}}$,
	puisque nous n'avons pas accès à $x(t)$. Le modèle à tracer sera
	\[
...
le tout a bien une concentration en $\ce{HO^-_{\aqu}}$ similaire à celle qu'on
...
\QR{%
	...
	On mesure
	\[
		\xul{\sigma_{\infty} = \SI{3.64}{mS.cm^{-1}}}
	\]
}
...
\QR{%
	Tracer puis expliquer l'allure décroissante de la courbe $\sigma = f(t)$.
}{%
	Corrigé disponible sur \texttt{Capytale}\ftn{\href{https://capytale2.ac-paris.fr/web/c/b344-4889885}{b344-4889885}}.
	\smallbreak
	\begin{isd}
		\begin{center}
			\includegraphics[width=\linewidth]{TP11_cinetique_sigma_brut.pdf}
		\end{center}
		\tcblower
		On perd des ions hydroxyde de grande conductivité pour gagner des \ce{RCOO-}
		de plus petite conductivité. La conductivité est donc décroissante.
	\end{isd}
}
\QR{%
	...
	\begin{center}
		\includegraphics[scale=.5]{TP11_cinetique_sigma_reglin.pdf}
	\end{center}
}

\enonce{%
	\begin{tcb}(rema)<lftt>{Régression}
		Il est commun d'avoir une régression qui ne passe pas du tout par les points
		de mesure. La plupart du temps, cela est dû à une mauvaise valeur de
		$\sigma_{\infty}$, divisée par 2 alors qu'il ne fallait pas.
	\end{tcb}
}%
...
\QR{%
	...
	\begin{gather*}
		\boxed{k = \frac{a}{c_0}}
		\qav a\text{ le coefficient directeur}
		\\\Ra
		\xul{k \approx \SI{8.78 \pm 0.16e-2}{mol^{-1}.L.s^{-1}}}
	\end{gather*}
}
...
\QR{%
	Rappeler la loi d'\textsc{Arrhénius}.
}{%
...
\QR{%
	...
	\begin{minipage}{0.45\linewidth}
		...
		On trouve une régression passant bien par les points avec $\ln A =
			\num{19.8}$ et
		\begin{gather*}
			\frac{E_a}{R} = \SI{6.62e3}{K}
			\\\Ra
			\xul{E_a = \SI{5.5e4}{J.mol^{-1}}}
		\end{gather*}
	\end{minipage}
	\begin{minipage}{0.55\linewidth}
		\begin{center}
			\includegraphics[width=\linewidth]{TP11_cinetique_reglin_Ea.pdf}
		\end{center}
	\end{minipage}
}
\vspace{-15pt}
