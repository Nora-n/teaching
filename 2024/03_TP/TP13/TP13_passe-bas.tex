...
\enonce{%
	...
	\begin{tcn}(rapp){Rappel mesure de déphasage}
		Supposons $e(t) = E_{m} \cos(\w t)$ sur la voie $Y_1$ et $s(t) = S_{m} \cos
			(\w t + \f)$ sur la voie $Y_2$ de l'oscillogramme ci-contre.  Le déphasage
		$\f$ entre deux signaux est un nombre appartenant à l'intervalle
		$\left]-\pi~; \pi\right]$. Il se mesure grâce à l'oscilloscope.
		\smallbreak
		\begin{isd}[righthand ratio=.4]
			\begin{enumerate}
				\item[b]{Déterminer $\abs{\Delta\f_{s/e}}$}~: pour cela, il
				      faut placer les curseurs verticaux de manière à déterminer le
				      décalage temporel $\Delta t$, puis $\abs{\Delta\f_{s/e}} =
					      \w \abs{\Delta{t}}$ (en rad).
				\item[b]{Déterminer le signe de $\Delta\f_{s/e}$}~:
				      pour cela, on cherche quelle courbe est en avance sur l'autre. Sur
				      l'oscillogramme ci-contre, $s$ est en retard sur $e$ puisqu'il
				      s'annule après $e$~: on en déduit $\Delta\f_{s/e} < 0$.
			\end{enumerate}
			\tcblower
			\begin{center}
...
\QR{%
	...
	\begin{center}
		...
		\captionof{figure}{Schéma complété.}
		\label{fig:rc-sysam}
	\end{center}
}
...
\enonce{%
	...
	\begin{tcb}[breakable](expe)<itc>{À la main}
		\begin{enumerate}
...
\QR{%
	...
	\vspace{-15pt}
	\begin{center}
		\captionof{table}{Mesures pour diagramme de \textsc{Bode}.}
		\begin{tabularx}{\linewidth}{YYYYY}
			\toprule
			$f$ (\si{Hz})                       &
			$G\ind{dB}$ (\si{dB})               &
			$\abs{\Delta{t}}$ (\si{\micro s})   &
