...
\QR[6]{%
	...
	\end{DispWithArrows*}
	\begin{gather*}
		\beforetext{Par identification, on a alors :}
		{\w_0}^2 = \frac{1}{LC}
		\qqMath{et}
		\frac{\w_0}{Q} = \frac{R}{L}
		\\\beforetext{Ainsi}
		\boxed{\w_0 \stm{=} \frac{1}{\sqrt{LC}}}
		\qqMath{et}
...
\QR[1]{%
	...
	$Q>1/2$ (précisément, c'est pour $Q=\num{0.72}$). Aucune des deux premières
	réponses A ou B n'est juste. Notons en revanche que pour $Q=1/2$, on a le
	transitoire le plus bref sans dépassement. Par ailleurs, le facteur de qualité
	s'écrivant
	\[
...
\QR[2]{%
	...
	\[
		\boxed{R \stm{=} \frac{1}{Q}\sqrt{\frac{L}{C}}}
		\Ra
		\xul{R = \SI{5}{\ohm}} \pt{1}
	\]
	Réponse \fbox{C}.
...
\QR[6]{%
	...
	\begin{gather*}
		...
		\\\beforetext{De discriminant}
		\Delta =
		\left(\frac{\w_0}{Q}\right)^2-4{\w_0}^2 \stm{=}
		{{\w}_0}^2\left(\frac{1}{Q^2}-4\right) \stm{<} 0
		\qMath{car}
		Q = 10 > \frac{1}{2}
		\\\beforetext{D'où les racines}
		r_\pm \stm{=}
		-\frac{\w_0}{2Q}\pm j \frac{\sqrt{-\Delta}}{2} =
		-\frac{\w_0}{2Q}\pm j \w_0 \sqrt{1-\frac{1}{4Q^2}}
		\intertext{%
			Que l'on peut identifier avec la forme des racines proposée par l'énoncé pour
			être en cohérence avec l'expression de $u_C\left(t\right)$~:
		}
...
\QR[1]{%
	...
	\begin{gather*}
		\beforetext{De même, par identification,}
		\W = \frac{\sqrt{-\Delta}}{2} \stm{=} \w_0\sqrt{1-\frac{1}{4Q^2}}
	\end{gather*}
	\medskip
	Réponse \fbox{A}.
}
...
\QR[1]{%
	...
	Exprimons la constante d'intégration $A$ à l'aide des conditions
	initiales déterminées à la question~\ref{q:1}~:
	\begin{gather*}
		u_C\left(t=0^+\right)=0=\exp(-0/\tau)\left[A\cos{\left(0\right)}+B\sin{\left(0\right)}\right]+E
		\\\Lra
		\boxed{A \stm{=} -E}
	\end{gather*}
	Réponse \fbox{B}.
