...
\QR[7]{\label{q:kapp}%
	...
	\pt{1}
	Avec les ordres partiels indiqués, on trouve
	\begin{align*}
		...
		\\\Lra
		\Aboxed{v(t)  & \stm{=} k\ind{app} [\ce{NO}](t)^p}
		\makebox[0pt][l]{\qav \boxed{k\ind{app} \stm{=} k [\ce{O_2}]_0^q}}
	\end{align*}
...
\QR[4]{%
	...
	\begin{align*}
		...
		\\\Lra
		\ln (k\ind{app,1}) - \ln (k\ind{app,2}) & \stm{=}
		q \left( \ln ([\ce{O_2}]_{01}) - \ln ([\ce{O_2}]_{02}) \right)
		\\\Lra
		\Aboxed{
		q                                       & \stm{=}
			\frac{%
...
\QR[3]{%
	...
	\begin{align*}
		...
		\\
		\Ra
		\makebox[0pt][l]{$\xul{\phantom{k = \SI{7.5e10}{mol^{-2}.L^{2}.s^{-1}}}}$}
