...
\QR[4]{%
Exprimer $X_m$ en fonction de $\w_0$, de $Q$ et des données. On pourra
utiliser la notation complexe.
}{
En notations complexes, on définit la représentation complexe
$\xul{x}(t)=X_me^{j(\w t+\f)}$ et l'amplitude complexe $\xul{X_m}=X_me^{j\f}$.
...
\begin{gather*}
	...
	\Lra
	\xul{X_m} \stm[-1]{=}
	\frac{\frac{-eE_0}{m}}{\w_0{}^{2}-\w^2 + \frac{(\jw) \w_0}{Q}}
	\\\Lra
	\boxed{
		\xul{X_m} \stm[-1]{=} \frac{-eE_0}{m\w_0{}^{2}}
		\frac{1}{1 -
...
\[
	\boxed{
		...
		\abs{\xul{X_m}} =
		\stm{=}
		\frac{eE_0}{m\w_0^2} \frac{1}
		{\sqrt{\left(1-\frac{\w^2}{\w_0^2} \right)^2 + \frac{\w^2}{Q^2\w_0^2}}}
	}
\]
}

\QR[3]{%
	Exprimer $\tan{\f}$ en fonction de $\w_0$ et de $Q$.
}{
	On a~:
	\begin{gather*}
		\xul{X_m} = \frac{eE_0}{m\w_0{}^{2}}
		\frac{1}{
			\left( \dfrac{\w}{\w_0} \right)^2 -1 +
			\jj\dfrac{\w}{Q\w_0}}
		\\\Ra
		\f \stm{=}
		\arg*{\xul{X_m}} =
		\underbracket[1pt]{\arg*{\frac{eE_0}{m\w_0{}^{2}}}}_{=0} -
		\arg*{\left( \dfrac{\w}{\w_0} \right)^2 -1 + \jj\dfrac{\w}{Q\w_0}}
		\\\Lra
		\tan(\f) \stm{=}
		\frac{-\frac{\w}{Q\w_0}}{\left( \frac{\w}{\w_0} \right)^{2} - 1}
		\Lra
		\boxed{
			\tan(\f) \stm[-1]{=} \frac{\w\w_0}{Q \left( \w_0{}^{2} - \w^{2} \right)}
		}
	\end{gather*}
}
...
\QR[1]{%
	...
	On trouve
	\[
		\boxed{\w_0 \stm[-1]{=} \SI{2,34e16}{\radian\per\second}}
	\]
}
...
\QR[2]{%
	...
	En comparant $\w$ et $\w_0$, on peut considérer que $\w_0 \stm{\gg} \w$ (il y
	a au moins un facteur 5 entre les 2, c'est un peu juste). De plus, $Q \gg 1$.
	Ainsi on peut simplifier le dénominateur du $\xul{X_m}$ car
	\[
		\frac{\w\w_0}{Q} \ll \w^2 \ll \w_0^2 \quad \pt{1}
	\]
	Dans ce cas,
	\[
...
\enonce{
	Un électron diffuse dans toutes les directions un rayonnement dont la
	puissance moyenne $\Pc$ est proportionnelle au carré de l'amplitude de son
	accélération.
}
...
\QR[2]{%
	...
	\[
		\xul{A_m} \stm{=} (\jw)^2 \xul{X_m}
		\quad \Rightarrow \quad
		A_m \stm{=} \frac{eE_0\w^2}{m\w_0^2}
	\]
	D'après le sujet, la puissance est proportionnelle au carré de l'amplitude de
	l'accélération, donc
	\[
...
\QR[2]{%
	...
	On peut comparer la puissance diffusée pour un rayonnement bleu avec un
	rayonnement rouge~:
	\[
		\boxed{\frac{P_b}{P_r} \stm[-1]{=} \frac{\w_b^2}{\w_r^2} \stm[-1]{=} 4}
	\]
	La puissance diffusée pour les rayonnements bleus est 4 fois plus importante
	que celle pour un rayonnement rouge, d'où la couleur du ciel~!
}

\QR[2]{%
	Pourquoi le ciel est-il rouge quand le Soleil se couche~?
}{%
	Le Soleil rasant parcourt une plus grand couche d'atmosphère \pt{1} par
	rapport au zénith~: tout le rayonnement bleu a déjà été diffusé, et il ne
	reste que le rouge. \pt{1}
}
