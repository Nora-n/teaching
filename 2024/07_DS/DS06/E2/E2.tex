...
\begin{document}
\exercice[23]{Toboggans\ifcorrige{~\small\textit{(D'après CCS TSI 2018)}}}
...
\enonce{
	...
	Une baigneuse de masse $m$ suit la trajectoire d’équation $r = R$, $z =\alpha
		\th$, l’axe $(Oz)$ étant orienté selon la \textbf{verticale descendante}.
}
...
\QR[5]{%
	...
	\begin{align*}
		...
		\Lra
		\xul{v\ind{fin} \stm[-1]{=} \SI{8.86}{m.s^{-1}}}
	\end{align*}
}
...
\QR[6]{%
	...
	\begin{DispWithArrows*}
		\dv{\Ec_m}{t} = 0
		\Lra
		m \big[(R^2 \tp \tpp) &+ (\alpha^2 \tp \tpp) \big] -mg\alpha \tp \stm{=} 0
		\Arrow{$\tp \neq 0$ constamment}
		\\\Lra
		\Aboxed{
