...
\QR{Que valent l’intensité $i\left(0^+\right)$ et la tension $u_C(0^+)$ à
	...
	\begin{tasks}[label=\protect\fbox{\Alph*}, label-width=4ex](4)
		...
		\task $i\left(0^+\right)=\f{E}{R}$
		\task $u_C\left(0^+\right)=0$
		\task $u_C\left(0^+\right)=E$
	\end{tasks}
}{
	Intéressons-nous d’abord au circuit à $t<0$.
	L’interrupteur est alors fermé si bien que $u_C$ est une tension aux bornes d’un fil donc
	...
	Par ailleurs en régime permanent constant, on sait que la bobine est équivalente à un interrupteur fermé
	(un fil). Si bien que le circuit est alors équivalent à uniquement la résistance $R$ en série avec la source idéale de fem $E$. Ainsi d’après la loi de Pouillet,
	...
	\centers{$i\left(t=0^+\right)=i\left(t=0^-\right)=\f{E}{R}$}
...
{Etablir l’équation différentielle vérifiée par $u_C(t)$ pour $t>0$.
	...
	\centers{$\f{\dd^2 u_C(t) }{\dd t^2}+\f{\w_0}{Q} \f{\dd u_C(t) }{\dd t}+ \w_0^2 u_C(t) = \alpha$}
...
}
{
	...
	\centers{$u_R\left(t\right)=Ri(t)$}

	\leftcenters{et la bobine}{ $u_L\left(t\right)=L\f{\dd i}{\dd t}$}

	\leftcenters{Il vient :}{$E=Ri+L\f{\dd i}{\dd t}+u_C$}

	Appliquons la relation courant-tension pour le condensateur :

	\centers{$i\left(t\right)=C\f{\dd u_C}{\dd t}$}

	\leftcenters{On obtient donc :}{$E=RC\f{\dd u_C}{\dd t}+LC\f{\dd^2u_C}{\dd t^2}+u_C$}

	Enfin, divisons cette expression par $LC$ pour mettre l’expression sous forme canonique.

	\centers{$\f{d^2u_C}{dt^2}+\f{R}{L}\f{du_C}{dt}+\f{u_C}{LC}=\f{E}{LC}$}

	\leftcenters{Par identification, on a alors :} {${\w_0}^2=\f{1}{LC}$}

	\leftcenters{ Ainsi }{$\w_0=\f{1}{\sqrt{LC}}\quad  \text{et} \quad \f{\w_0}{Q}=\f{R}{L}$}

	\leftcenters{Soit}{$Q=\f{L\w_0}{R}=\f{L}{R}\f{1}{\sqrt{LC}}=\f{1}{R}\sqrt{\f{L}{C}}$}
...
}
{
	...
	\centers{$R=\f{1}{Q}\sqrt{\f{L}{C}}=\f{1}{10}\sqrt{\f{50\times{10}^{-3}}{20\times{10}^{-6}}}=\SI{5}{\w}$}
...
\centers{$u_C\left(t\right)=\exp{\left(- \f{t}{\tau}\right)}\left[A\cos{\left(\Wt\right)+B\sin{\left(\Wt\right)}}\right]+u_{C,p}$}
}
...
\centers{$s^2+\f{\w_0}{Q}s+{\w_0}^2=0$}

\leftcenters{De discriminant :} {$\Delta=\left(\f{\w_0}{Q}\right)^2-4{\w_0}^2={{\w}_0}^2\left(\f{1}{Q^2}-4\right)<0 \quad \text{car} \quad Q=10>\f{1}{2}$}

Les racines s’écrivent donc, avec  $j^2=-1$,

\centers{$r_\pm=-\f{\w_0}{2Q}\pm j \f{\sqrt{-\Delta}}{2} =-\f{\w_0}{2Q}\pm j \w_0 \sqrt{1-\f{1}{4Q^2}}$}
...
\centers{ $r_\pm=-\f{1}{\tau}\pm j\W$ }

\leftcenters{Ainsi, par identification,}             {$\tau=\f{2Q}{\w_0}$}
...
{
	\leftcenters{De même, par identification, }{$\W=\f{\sqrt{-\Delta}}{2}=\w_0\sqrt{1-\f{1}{4Q^2}}$}
...
\leftcenters{D’où : }{$0+0+\f{u\e{CP}}{LC}=\f{E}{LC} \quad \text{d'où} \quad u\e{CP}=E$}
...
}
{
	...
	\centers{$u_C\left(t=0^+\right)=0=exp\left(-0/\tau\right)\left[A\cos{\left(0\right)}+B\sin{\left(0\right)}\right]+E$}
...
}
{
	...
	\centers{${\dot{u}}_C\left(t=0^+\right)=\f{i\left(t=0^+\right)}{C}=\f{E}{RC}$}
	...
	\leftcenters{ Ainsi }{$\WB=\f{E}{RC}+\f{A}{\tau}$}
	...
	\leftcenters{De plus} {$\tau=\f{2Q}{\w_0}=\f{2}{R}\sqrt{\f{L}{R}}\times\sqrt{LC}=\f{2L}{R}$}
