...
\chapter{\cswitch{%
	  ...
	  Dosage par titrage acidobasique avec suivi pH-m\'etrique et
		...
		\begin{tcb}*(ror)"how"{Savoir-faire}
			\begin{itemize}
				...
				\item Identifier et exploiter la réaction support du titrage
				      (recenser les espèces présentes dans le milieu au cours du  titrage, repérer
...
\QR{%
	...
	\begin{gather*}
		...
		\begin{array}{rcl}
			m_0              & = & \SI{8}{g}
			\\
			\rho_V           & = & \SI{1.0}{g.cm^{-3}} = \SI{1.0e3}{g.L^{-1}}
			\\
		...
		\beforetext{Pour l'équivalence~:}
		c_0V_A = c_B V\ind{equi}
		\Lra
		V\ind{equi} = \frac{c_0V_A}{c_B} = \SI{130}{mL}
	\end{gather*}
	Il faut donc \textbf{diluer par 10} pour avoir un volume équivalent aux
	alentours de \SI{13}{mL}, soit la moitié de la burette. D'où le protocole~:
	\begin{itemize}
...
\QR{%
	...
	\begin{center}
		\includegraphics[width=.8\linewidth]{vinaigre_verrie-fig1}
	\end{center}
}
...
\begin{tcb}(expe){Tracé de la courbe}
	\begin{enumerate}
...
\QR{%
	...
	\begin{center}
		\includegraphics[width=.8\linewidth]{vinaigre_titrage-fig1}
	\end{center}
}
...
\setcounet{subsubsection}{1}

\subsubsection{Dosage}

\begin{tcb}*(expe)"chem"{Dosage}
	\begin{enumerate}
